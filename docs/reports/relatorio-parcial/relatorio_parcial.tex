...
Neste relatório parcial, descrevemos as atividades realizadas nos 2 primeiros meses da bolsa de Iniciação Científica, dedicadas ao estudo e implementação de algoritmos para o Problema de Visita de Polígonos (\textit{Touring Polygons Problem}, TPP). O TPP consiste em determinar um caminho de comprimento mínimo na norma Euclidiana partindo de um ponto inicial~\(s\), visitando uma sequência ordenada de polígonos~\(P_1, \dots, P_k\) no plano e terminando em um ponto~\(t\). Em sua formulação geral, o problema é NP-difícil; por esse motivo, focamos o período coberto por este relatório na variante em que os polígonos são convexos e disjuntos, para a qual existe solução polinomial.

Estudamos o artigo de Dror et al.~\cite{tpp-dror-2003}, que introduz o problema e apresenta um algoritmo exato baseado nas estruturas de região de primeiro contato e mapa de último passo, com complexidade~\(O(nk\log(n/k))\), onde~\(n\) é o número total de vértices e~\(k\) é o número de polígonos. Implementamos esse algoritmo em C++23, produzindo pseudocódigo detalhado, análise de complexidade e experimentos computacionais que confirmam o comportamento assintótico previsto. Desenvolvemos também uma ferramenta de visualização interativa, disponível em \url{https://yushipy.github.io/PersonalPage/Visualizer/index.html}, que permite inspecionar instâncias, soluções e mapas de último passo, e que estenderemos para suportar o caso não convexo nas próximas etapas do projeto.
...
O TPP apresenta variantes de dificuldade bastante distinta. Quando os polígonos são convexos e disjuntos, o problema admite solução exata em tempo polinomial: Dror et al.~\cite{tpp-dror-2003} apresentam um algoritmo de complexidade~\(O(nk\log(n/k))\), onde~\(n\) é o número total de vértices. Quando os polígonos podem ser não convexos, o problema torna-se NP-difícil, mesmo com a ordem de visita fixada~\cite{tpp-dror-2003}.

Este relatório cobre os 2 primeiros meses da bolsa, dedicados ao estudo e implementação do caso convexo disjunto, que é o mais simples e serve de base para as etapas seguintes do projeto. Estudamos em detalhe o algoritmo de Dror et al.~\cite{tpp-dror-2003}, baseado nas estruturas de região de primeiro contato e mapa de último passo. Implementamos o algoritmo em C++23 com busca binária para localização de pontos nos mapas de último passo, atingindo a complexidade~\(O(nk\log(n/k))\) prevista no artigo original. Realizamos experimentos computacionais para verificar o comportamento assintótico e comparar variantes da implementação. Desenvolvemos também uma ferramenta de visualização interativa que permite inspecionar instâncias, soluções e mapas de último passo.
...
Consideramos o TPP na variante em que os polígonos são convexos e disjuntos. Utilizamos tanto a formulação quanto a solução apresentadas em~\cite{tpp-dror-2003} como base para a implementação desenvolvida. O problema é definido formalmente a seguir:
...
Utilizamos o operador~\(\times\) para denotar o produto vetorial em~\(\mathbb{R}^2\): para dois vetores~\(a = (a_x, a_y)\) e~\(b = (b_x, b_y)\), define-se~\(a \times b = a_x b_y - a_y b_x\). Note que~\(a \times b > 0\) se~\(b\) está à esquerda de~\(a\), e~\(a \times b < 0\) se~\(b\) está à direita de~\(a\).
...
O algoritmo opera em duas fases. Na primeira, para cada~\(i = 1, \dots, k\), calcula-se~\(T_i\) e em seguida~\(S_i\), usando os mapas já computados. Na segunda, consulta-se o~\(k\)-path até~\(t\). A seguir, detalhamos cada etapa.
...
Com~\(S_i\) calculado, o passo central é localizar um ponto~\(p \in \mathbb{R}^2\) no mapa~\(S_i\). Descrevemos a seguir como realizar cada verificação em tempo~\(O(1)\).
...
Analisamos a complexidade de cada componente do algoritmo, de baixo para cima.
...
A ferramenta foi projetada para acompanhar o desenvolvimento do projeto: à medida que implementarmos o suporte ao caso não convexo nas etapas seguintes, estenderemos a visualização para cobrir essas instâncias. No momento da escrita deste relatório, instâncias com polígonos não convexos desativam automaticamente a exibição dos mapas de último passo. A página explicativa associada à versão histórica da ferramenta não faz mais parte do produto mantido.

\section{Plano de Atividades para o Próximo Período}
\label{sec:proximos_passos}

Dedicaremos o próximo período ao TPP não convexo com ordem fixa, que é NP-difícil~\cite{tpp-dror-2003}. A abordagem planejada consiste em decompor cada polígono não convexo em peças convexas, usando o algoritmo de Greene~\cite{cpd-greene-1983} disponível na biblioteca CGAL~\cite{cgal}, e aplicar o algoritmo desenvolvido neste período a cada combinação possível de peças, explorando técnicas de Branch and Bound para tornar essa busca tratável na prática.
...
\begin{enumerate}
	...
	\item Formular e implementar o algoritmo de Branch and Bound para o TPP não convexo. Cada nó da árvore de busca atribui uma peça convexa a cada polígono; o limitante inferior é obtido substituindo os polígonos ainda não atribuídos pelos seus cascos convexos e resolvendo o TPP convexo resultante, o que mantém o cálculo do limitante em tempo polinomial em cada nó.
