...
\begin{figure}[h]\centering\figcrossbend\caption{Exact-coordinate crossing and vertex-bend examples for \(P=[0,2]\times[0,2]\). The crossing route uses \(s=(-2,1)\), \(q=(3,1)\); the bend route uses \(s=(-2,-1)\), \(v=(0,0)\), and \(q=(1,-2)\). Half-unit grids make the boundary contact and the equal x/y scales explicit.}\label{fig:cross-bend}\end{figure}
