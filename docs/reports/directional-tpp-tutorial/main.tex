...
\usepackage{caption,fancyhdr,hyperref,enumitem,needspace,float}
...
\newcommand{\Rect}[2]{\operatorname{Rect}\!\left[#1,#2\right]}
...
{\sffamily\scriptsize\bfseries\color{good}READING MAP \hfill 14 SECTIONS \quad\textbullet\quad 21 FIGURES}\par
\vspace{2mm}
...
\section*{The solver in one page}
\phantomsection
\addcontentsline{toc}{section}{The solver in one page}
The input asks for a shortest route that visits closed convex regions in a fixed order. Each level adds one polygon \(P_i\); the map at that level chooses only the \emph{last} action. Earlier levels recover the prefix recursively.

\begin{figure}[H]\centering\figoverview
\caption{The implemented last-step recursion for an ordinary point query \(q\). Inherited crossing includes closed membership and exterior pass-through cells. Ray construction uses symbolic one-sided queries, while path refolding checks the finite edge. This is an action guide, not a proof that the implicit cells cover every degenerate input.}\label{fig:overview}
\end{figure}

The next two sections establish the visit rule and the three actions geometrically. Section~\ref{sec:execution} and the coordinate-arithmetic section trace a complete two-reflection query. The general map-equivalence proof obligation remains explicit in the final discussion.
\clearpage
...
The order condition applies to the \emph{complete path}, not to the set of polygons it happens to intersect. Figure~\ref{fig:order} makes the difference sharp: the shorter line meets \(P_2\) first and \(P_1\) later, but is invalid for the requested \(P_1,P_2\) order. A geometric segment may itself satisfy several visits: the solver need not store a vertex at every entry. One may choose ordered parameters along that segment, including equal parameters at a shared boundary point.

\begin{figure}[h]\centering\figordered\caption{The dashed path is shorter than the solid path, but visits the polygons in an incorrect order.}\label{fig:order}\end{figure}
...
\end{equation}
Why does this formula give the optimum? Every feasible route is at least as long as the expression on the right: mark its \(i\)th visit at \(x\), then its prefix costs at least \(F_{i-1}(x)\) and its suffix at least \(\norm{q-x}\). Conversely, take a shortest prefix to a chosen \(x\in P_i\) and append the straight segment \(xq\); that route is feasible and attains the corresponding cost, even if the segment meets earlier polygons again. The polygons include their boundaries and are bounded, so a best contact cannot escape to infinity or approach an excluded boundary point. Repeating this argument gives a shortest polygonal route with at most one chosen contact per visit before collinear contacts are removed.

The \emph{path} is a geometric polyline; \(F_i(q)\) is only its scalar length. Its \emph{last segment} is the final nonzero straight piece, which may be absent at a zero-length contact.

Repeated visits illustrate why a shortest route and its visit schedule are different objects. Suppose \(P_1,P_2,P_3\) are the same square. If the segment \(sq\) passes through that square, all three visits can be assigned to its entry point, or to three successive points along the segment; the \emph{route stays the same}. If \(q\) lies inside the square, all three visits can occur at the endpoint. Thus a returned polyline need not contain a separate vertex for each required visit, and its vertex count is not a visit log.

\begin{figure}[h]\centering\figrepeat\caption{Three coincident input squares. Left: the segment \(sq\) already supplies all three visits. Right: because \(q\) lies inside all three squares, all visits may occur at the endpoint.}\label{fig:repeat}\end{figure}

For coordinate examples, \(\Rect{(a,b)}{(c,d)}\) means the closed axis-aligned rectangle with lower-left corner \((a,b)\) and upper-right corner \((c,d)\). We use corner pairs so the placement and dimensions of each rectangle are immediately visible.

An old false-crossing failure shows the opposite trap. Take
\[
\begin{aligned}
s&=(-4,-4),&t&=(4,-4),\\
P_1&=\Rect{(0,2)}{(2,4)},&P_2&=\Rect{(0,-2)}{(3,2)},\\
P_3&=\Rect{(-1,3)}{(1,4)}.
\end{aligned}
\]
Thus \(P_1\) is the upper-right blue rectangle (width 2, height 2), \(P_2\) the lower-right ochre rectangle (width 3, height 4), and \(P_3\) the narrow green rectangle across the top (width 2, height 1). In particular \((0,3)\in P_1\cap P_3\), whereas \((0,2)\in P_1\cap P_2\).
...
\begin{figure}[h]\centering\figfalsecrossing\caption{The exact three-rectangle false-crossing instance: blue \(P_1\), ochre \(P_2\), green \(P_3\). Arrows show route direction. The old dashed route reaches \((0,3)\) in \(P_1\cap P_3\) and only later reaches \(P_2\) at \((4/7,2)\), so its encounter order is wrong. The corrected solid route visits \(P_1\) and \(P_2\) together at \((0,2)\), then \(P_3\) at \((1/2,3)\). Both panels use equal x/y scales.}\label{fig:false-crossing}\end{figure}

The full view preserves the source and target of the historical fixture. Figure~\ref{fig:false-crossing-zoom} magnifies the contacts instead of moving \(s\), which would change the exact recorded instance.
\begin{figure}[H]\centering\figfalsecrossingzoom\caption{Close view of the same two routes near the three rectangles. Red is the old invalid route; green visits \(P_1\) and \(P_2\) simultaneously before \(P_3\). Both complete routes still start at \(s=(-4,-4)\) and end at \(t=(4,-4)\), outside this crop. The magnification makes the different contact order visible without changing the fixture.}\label{fig:false-crossing-zoom}\end{figure}
...
\begin{figure}[h]\centering\figcontacts\caption{Three optimal tours from the same source to different targets around one square: straight crossing, bending at a vertex, and reflection at an edge interior. Equal x/y scales preserve the local geometry.}\label{fig:contacts}\end{figure}
...
For an explicit one-polygon reflection, use \(P=\Rect{(0,0)}{(2,2)}\), \(s=(-2,1)\), and \(q=(-1,0)\). The straight \(sq\) stays to the left of \(P\). Reflecting \(q\) in the supporting line \(x=0\) gives \(q'=(1,0)\). The segment from \(s\) to \(q'\) reaches \(x=0\) after \(2/3\) of its length, at \(c=(0,1/3)\), which is inside the finite left edge. Thus \(s\to c\to q\) is a feasible edge-reflection tour of length \(\norm{s-q'}=\sqrt{10}\). This does not merely show equal angles; it demonstrates why the recursive algorithm queries the preceding map at \(q'\) and later replaces its final unfolded portion by \(cq\).

\begin{figure}[h]\centering\figreflection\caption{Exact one-polygon reflection example. In the physical panel, the route is \(s\to c\to q\) with \(c\) on the finite edge \(x=0\). In the unfolded panel, reflecting \(q\) to \(q'\) makes the same route a straight segment, and its intersection with \(x=0\) is \(c=(0,1/3)\). Both panels use equal x/y scales and a half-unit grid padded around the geometry. The labels mark points; the preceding paragraph gives their exact coordinates.}\label{fig:reflection-example}\end{figure}
...
\begin{figure}[h]\centering\figcrossbend\caption{Exact-coordinate crossing and vertex-bend examples for \(P=\Rect{(0,0)}{(2,2)}\). The crossing route uses \(s=(-2,1)\), \(q=(3,1)\); the bend route uses \(s=(-2,-1)\), \(v=(0,0)\), and \(q=(1,-2)\). Half-unit grids make the boundary contact and the equal x/y scales explicit.}\label{fig:cross-bend}\end{figure}
...
Figure~\ref{fig:last} shows a complete \emph{one-polygon} map, where \(F_0(q)=\norm{q-s}\). For \(s=(-2,1)\) and \(P_1=\Rect{(0,0)}{(2,2)}\), the two rays from \(s\) through \((0,0)\) and \((0,2)\) bound the right-hand straight-crossing region. On the left, reflecting \(q\) across \(x=0\) puts the contact on the finite edge precisely when \(x/2\le y\le2-x/2\). Outside those bounds the optimal contact is the lower or upper left vertex. This is an exact subdivision for this one polygon; the later intersecting-map construction needs additional split vertices and incident limits.

\begin{figure}[h]\centering\figlastmap\caption{Complete last-step map for \(s=(-2,1)\) and \(P_1=\Rect{(0,0)}{(2,2)}\), within the displayed window. Green is crossing, ochre is reflection on the left edge, and rose is bending at a left vertex. The four labeled targets show one optimal route in each region. Dashed rays are exact cell boundaries, and equal x/y scales preserve the geometry. This simple map does not assert a complete subdivision for intersecting inputs.}\label{fig:last}\end{figure}
...
The binary-search step is geometric, not merely numerical indexing. Let \(L\) be the current left boundary vertex and \(J\) the middle tested vertex. First test \(J\)'s cone. If it contains \(q\), return \(2J\). Otherwise \code{point\_in\_edge\_plus(q,L,J,L.after,J.before)} asks whether the cell source lies on the boundary interval from \(L\) through \(J\). If yes, discard the interval after \(J\) by setting \code{right=mid}; if no, discard the interval through \(J\) by setting \code{left=mid+1}. The invariant is that a circular interval of candidate following edges remains. At termination the following edge at \code{left} is selected. The disjoint implementation then checks first-contact flags. Its binary search partitions the full boundary fan, including regions whose preceding optimal route already visits the current polygon. A candidate with no incident first-contact edge is a pass-through cell and now returns \(-1\) directly. The former linear scan returned \(-1\) on every observed fallback in a 100,000-instance seeded disjoint probe; removing it preserved a bitwise hash of every returned route and the legacy test corpus. This is empirical support for the direct classification, while a complete proof of the C++ predicates' circular ordering and floating-point boundary behavior remains separate.

Section~\ref{sec:execution} applies the same cone-and-chord search to an actual split, intersecting polygon; Figure~\ref{fig:binary} shows the candidate interval shrinking there. Its chords are genuine predicate tests, not asserted boundaries of a complete shortest-path subdivision.
...
\begin{sourcebox}
\textbf{Open the code:} \code{SOL:146--188} builds the two vertex rays and first-contact flags; \code{BS:143--200} performs the circular binary search and crossing classification; \code{GEO:67--111} defines its floating-point cone and chord predicates. Workspace initialization is \code{SOL:91--132}.
\end{sourcebox}
...
Figure~\ref{fig:witness-setup} defines the two-polygon witness visually. The labels give the source, targets, polygon corners, and two boundary crossings. The blue rectangle is \(P_1=\Rect{(0,-6)}{(6,6)}\); the ochre polygon \(P_2\) lies above \(y=x/2\), below \(y=9\), and between \(x=-8\) and \(x=8\).
\begin{figure}[H]\centering\figwitnesssetup\caption{The full exact two-polygon witness before unfolding. Blue \(P_1\) is specified by its opposite labeled corners. Ochre \(P_2\) is bounded by the labeled lower-edge endpoints \((-8,-4),(8,4)\), the horizontal top \(y=9\), and vertical sides. The source, both targets, and the crossings are labeled directly. The supporting lines \(\ell_1,\ell_2\) are highlighted; equal axis scales preserve the geometry.}\label{fig:witness-setup}\end{figure}
The left supporting edge of \(P_1\) is \(\ell_1:x=0\), and the lower supporting edge of \(P_2\) is \(\ell_2:y=x/2\). The polygon boundaries cross at \(j\) and \((6,3)\). The unique prefix path to \(j\) is \(s\to j\), with unit incoming direction \(u=(3/5,4/5)\). The reflection matrices are
\[
...
\begin{figure}[htbp]\centering\figsymbolic\caption{The exact sign rule for the example above. Three zero coefficients defer the decision to \(a\times e_2=+1\); the boxes show symbolic priority, not four finite geometric displacements.}\label{fig:symbolic}\end{figure}
...
The similarly named code queries now have different roles. The disjoint solver's \code{Solution::query(point,i)} returns the preceding point for its last segment. The intersection solver's \code{virtual\_source(Query,level)} returns the unfolded directional point \(V\), which is usually not a physical contact. Its \code{query\_path} reconstructs a route, while \code{query\_length} returns a scalar length. A caller cannot substitute one of these results for another at a boundary intersection.
\begin{sourcebox}
\textbf{Open the code:} \code{SOL:192--286} contains the disjoint path and last-contact queries; \code{IM:196--274} contains virtual-source, path, and length queries for intersections.
\end{sourcebox}
...
\begin{figure}[htbp]\centering\figbinary\caption{The split \(P_2\) boundary and the three chord tests for \(q=t_B\). The dashed chord joins \(v_0\) to the tested endpoint; its green ray is scaled by a positive factor. Green boundary intervals remain candidates, red intervals are discarded. All three predicates return true, shrinking \([0,5]\to[0,2]\to[0,1]\to[0,0]\). Each panel has equal axis scales.}\label{fig:binary}\end{figure}

Here \(m=6\): the locator tests \(v_0\)'s cone, then those at \(v_3,v_2,v_1\); none contains \(t_B\). The three true chord tests retain the left circular interval and select following pseudo-edge \(e_0\), location code \(1\).
...
\begin{figure}[htbp]\centering\figrefoldtrace\caption{The recursive query transforms go from level 2 to level 0 (top); physical contacts are restored in reverse order (bottom). The arrows between boxes indicate calls and returns, not route segments. Each finite-edge check is detailed in Section~\ref{sec:execution} and the arithmetic section below.}\label{fig:refold-trace}\end{figure}
...
Public dispatch and winding&\code{normalized\_winding}; pairwise test; public entry points&\code{BS:90--126, 201--300}\\
Disjoint binary and crossing&\code{\_locate\_point}; \code{locate\_point}&\code{BS:143--200}\\
