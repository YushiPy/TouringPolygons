...
Para determinar~\(T_i\), basta verificar cada aresta de~\(P_i\). Pelo Lema~\ref{lemma:first_contact}, uma aresta~\(e = \overline{uv}\) pertence a~\(T_i\) se e somente se o ponto~\(q\) que precede~\(u\) no~\((i-1)\)-path até~\(u\) está do lado externo de~\(e\).

\begin{blockQuote}
\begin{lemma}
Seja~\(e = \overline{uv}\) uma aresta de~\(P_i\) e~\(q\) o ponto que precede~\(u\) no~\((i-1)\)-path até~\(u\). Tem-se que~\(e \in T_i\) se e somente se~\((v - u) \times (q - u) < 0\). \label{lemma:first_contact}
\end{lemma}
\end{blockQuote}

Essa ideia é implementada no Algoritmo~\algref{alg:first_contact_region}, ilustrado na Figura~\ref{fig:first_contact_region}. Os algoritmos a seguir fazem uso de uma função auxiliar \texttt{ConsultaÚltimoPasso}\((p, i, \dots)\), que dado um ponto~\(p \in \mathbb{R}^2\) e um índice~\(i\), retorna o ponto que precede~\(p\) no~\(i\)-path até~\(p\). Essa função será implementada na Seção~\ref{sec:query}, após a definição de \texttt{LocalizaPonto}.
...
\begin{figure}[!htb]
	...
	\begin{subfigure}[b]{0.45\textwidth}
		...
		\caption{\(e^1\) pertence a~\(T_2\):~\(q\) está do lado externo.}
	\end{subfigure}
	\hspace{10pt}
	\begin{subfigure}[b]{0.45\textwidth}
		\centering
		\includegraphics[width=\textwidth]{aresta-1.png}
		\caption{\(e^2\) não pertence a~\(T_2\):~\(q\) está do lado interno.}
	\end{subfigure}
	\vspace{1em}
	\begin{subfigure}[b]{0.45\textwidth}
		\centering
		\includegraphics[width=\textwidth]{aresta-2.png}
		\caption{\(e^3\) não pertence a~\(T_2\):~\(q\) está do lado interno.}
	\end{subfigure}
	\hspace{10pt}
	\begin{subfigure}[b]{0.45\textwidth}
		\centering
		\includegraphics[width=\textwidth]{arestas-resultado.png}
		\caption{Resultado: apenas~\(e^1\) pertence a~\(T_2\).}
	\end{subfigure}
	\caption{Determinação de~\(T_2\) pelo Algoritmo~\ref{alg:first_contact_region}.}
...
Com~\(S_i\) calculado, o passo central é localizar um ponto~\(p \in \mathbb{R}^2\) no mapa de último passo~\(S_i\). Para isso, é necessário verificar se~\(p\) pertence a uma região de vértice, a uma região de aresta ou à região de atravessar. Descreve-se a seguir como realizar cada uma dessas verificações em tempo~\(O(1)\).

\paragraph{Região de vértice.} Seja~\(v\) um vértice de~\(P_i\) com raios associados~\((r^1, r^2)\) em~\(S_i\). O ponto~\(p\) pertence à região de~\(v\) se e somente se está no cone definido por~\(v\) e pelos raios~\(r^1\) e~\(r^2\), em sentido anti-horário de~\(r^1\) a~\(r^2\). Dependendo do ângulo de abertura do cone, a condição é:

\begin{itemize}
	\item Se o ângulo é inferior a~\(180^\circ\), isto é,~\(r^1 \times r^2 \ge 0\): o ponto~\(p - v\) deve estar à esquerda de~\(r^1\) e à direita de~\(r^2\), ou seja,~\(r^1 \times (p - v) > 0\) e~\(r^2 \times (p - v) < 0\).
	\item Se o ângulo é superior a~\(180^\circ\), isto é,~\(r^1 \times r^2 < 0\): basta que~\(p - v\) esteja à esquerda de~\(r^1\) ou à direita de~\(r^2\), ou seja,~\(r^1 \times (p - v) > 0\) ou~\(r^2 \times (p - v) < 0\).
\end{itemize}

Esses dois casos estão ilustrados na Figura~\ref{fig:point_in_cone}.
...
\begin{figure}[!htb]
	...
	\end{subfigure}
	\caption{Verificação se~\(p\) está no cone definido por~\(v\) e pelos raios~\(r^1\) e~\(r^2\).}
	\label{fig:point_in_cone}
\end{figure}

\paragraph{Região de aresta.} Seja~\(e = \overline{uv}\) uma aresta de~\(P_i\) em~\(T_i\). O ponto~\(p\) pertence à região de~\(e\) se está do lado externo de~\(e\) e entre o segundo raio de~\(u\) e o primeiro raio de~\(v\) em~\(S_i\). Formalmente, sendo~\(r^1\) o segundo raio de~\(u\) e~\(r^2\) o primeiro raio de~\(v\):
\[
	r^1 \times (p - u) > 0 \quad \text{e} \quad r^2 \times (p - v) < 0 \quad \text{e} \quad (v - u) \times (p - u) < 0.
\]
Esse caso está ilustrado na Figura~\ref{fig:point_in_edge_region}.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.8\textwidth]{edge_region_check.png}
	\caption{Verificação da pertinência de~\(p\) à região associada à aresta~\(e = \overline{uv}\).}
...
Realizou-se uma série de experimentos com o objetivo de confirmar o comportamento assintótico previsto pela análise de complexidade. Os experimentos foram conduzidos em instâncias sintéticas controladas, parametrizadas por dois valores: o número de polígonos~\(k\) e o número de vértices por polígono~\(m\). Para cada configuração, geraram-se instâncias com~\(k\) polígonos regulares de~\(m\) vértices cada, dispostos de forma que todo ponto de~\(P_i\) esteja na região de atravessar de~\(P_j\) para todo~\(j < i\). Essa configuração garante que toda consulta percorre todos os mapas de último passo anteriores, representando o pior caso para o algoritmo. Em particular, tem-se~\(n = mk\), de modo que a complexidade teórica nesse cenário é~\(O(m\log(m) \cdot k^2)\).

Realizaram-se dois experimentos. No primeiro, fixou-se~\(m \in \{3, 5, 10, 20, 50, 100\}\) e variou-se~\(k\) de~\(1\) até~\(3000\), com~\(300\) medições uniformemente espaçadas. No segundo, fixou-se~\(k \in \{3, 5, 10, 20, 50, 100\}\) e variou-se~\(m\) de~\(3\) até~\(40000\), com~\(400\) medições uniformemente espaçadas.

Os resultados do primeiro experimento estão apresentados na Figura~\ref{fig:plt_vs_k}. Observa-se que o algoritmo apresenta crescimento quadrático em~\(k\) para todos os valores de~\(m\), conforme previsto. Para valores pequenos de~\(m\), o comportamento logarítmico em~\(m\) tem pouco impacto e as curvas são dominadas pelas constantes de implementação.
...
\begin{figure}[!ht]
	...
	\end{subfigure}
	\caption{Tempo de execução em função de~\(k\) para diferentes valores de~\(m\).}
	\label{fig:plt_vs_k}
\end{figure}

Os resultados do segundo experimento estão apresentados na Figura~\ref{fig:plt_vs_m}. Observa-se que o tempo de execução cresce de forma aproximadamente linear em~\(m\) para todos os valores de~\(k\), o que é consistente com o fator~\(m\log(m)\) da complexidade teórica, já que~\(\log(m)\) cresce lentamente e não é perceptível na escala dos gráficos. Esses resultados confirmam que a implementação atinge o comportamento assintótico previsto.
...
\begin{figure}[!ht]
	...
	\end{subfigure}
	\caption{Tempo de execução em função de~\(m\) para diferentes valores de~\(k\).}
	\label{fig:plt_vs_m}
\end{figure}
...
O próximo período será dedicado ao TPP não convexo com ordem fixa, que é NP-difícil~\cite{tpp-dror-2003}. A abordagem planejada consiste em decompor cada polígono não convexo em peças convexas e aplicar o algoritmo desenvolvido neste período a cada combinação possível de peças, explorando técnicas de Branch and Bound para tornar essa busca tratável na prática.

As atividades previstas são as seguintes:

\begin{enumerate}
	\item Implementar a decomposição convexa dos polígonos não convexos. Utilizará o algoritmo de Greene~\cite{cpd-greene-1983}, disponível na biblioteca CGAL~\cite{cgal}, que produz uma decomposição com número mínimo de peças convexas sem o uso de pontos de Steiner.
	...
	\item Implementar um incumbente heurístico para inicializar o Branch and Bound com uma solução de boa qualidade, reduzindo o espaço de busca explorado.
