...
start = Vector2(0.4, 0)
polygon = regular_polygon(Vector2(4, -4), 2, 3, rotation=0.3)

from UnconstrainedTPP.u_tpp_fast_jit import tpp_solve

bbox = get_bbox([start] + polygon, square=True, scale=1.5)
...
ax.fill([bbox[0], bbox[2], bbox[2], bbox[0]], [bbox[1], bbox[1], bbox[3], bbox[3]], color='lightgray', alpha=0.5)
ax.scatter(*start, color='black', edgecolor='white', s=80, zorder=5, linewidth=2)
ax.text(*(start + Vector2(-0.4, 0)), '$s$', color='black', fontsize=18, ha='center', va='center', zorder=6)

sol = Solution(start, start, [polygon])
sol.solve()
blocked = sol.blocked[0]
...
for i, v in enumerate(polygon):
	...
	p = v + (r1 + r2 * 0.8).scale_to_length(1.5)
	path = Solution(start, p, [polygon]).solve()
	ax.plot(*zip(*path), color='red', linestyle='--', linewidth=2, zorder=4)
	ax.scatter(*p, color='red', edgecolor='white', s=80, zorder=5, linewidth=2)
...
for i in range(len(polygon)):
	...
	p = v1.lerp(v2, 0.6) + (v2 - v1).perpendicular().scale_to_length(-1.8)
	path = Solution(start, p, [polygon]).solve()
	ax.plot(*zip(*path), color='green', linestyle='--', linewidth=2, zorder=3)
	ax.scatter(*p, color='green', edgecolor='white', s=80, zorder=5, linewidth=2)

star = next(i for i in range(len(polygon)) if blocked[i] and not blocked[(i - 1) % len(polygon)])
end = next(i for i in range(len(polygon)) if blocked[i] and not blocked[(i + 1) % len(polygon)]) + 1

v1 = polygon[star]
r1 = sol.cones[0][star][1]
v2 = polygon[end]
r2 = sol.cones[0][end][0]
...
va = polygon[m - 1]
v = polygon[m]
vb = polygon[(m + 1) % len(polygon)]

p = v + (v - va).scale_to_length(0.8) + (v - vb).scale_to_length(0.1)
path = Solution(start, p, [polygon]).solve()
ax.plot(*zip(*path), color='blue', linestyle='--', linewidth=2, zorder=4)
ax.scatter(*p, color='blue', edgecolor='white', s=80, zorder=5, linewidth=2)

ax.fill(*zip(*polygon), alpha=1, color="white", zorder=1)
ax.fill(*zip(*polygon), alpha=0.5, zorder=2, edgecolor='blue', linewidth=2)

center = sum(polygon, Vector2(0, 0)) * (1 / len(polygon))
ax.text(*center, f'$P_1$', color='black', fontsize=18, ha='center', va='center', zorder=6)


ax.fill([-1], [-1], color='red', alpha=0.5, label='Vértice')
ax.fill([-1], [-1], color='green', alpha=0.5, label='Aresta')
ax.fill([-1], [-1], color='blue', alpha=0.5, label='Atravessar')
