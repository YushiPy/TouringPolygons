...
polygons = [
    regular_polygon(Vector2(2, 7), 2, 4, rotation=0.7),
	regular_polygon(Vector2(4, -1), 2, 6),
]

from UnconstrainedTPP.u_tpp_fast_jit import tpp_solve
path = tpp_solve(start, target, polygons)

bbox = get_bbox([start, target] + [v for poly in polygons for v in poly], square=True, scale=1.5)
...
ax.scatter(*target, color='red', label='Target', edgecolor='white', s=80, zorder=5, linewidth=2)
ax.text(*(target + Vector2(0.7, 0)), '$t$', color='black', fontsize=18, ha='center', va='center', zorder=6)

for i, poly in enumerate(polygons):
    ax.fill(*zip(*poly), alpha=0.7, label=f'Polygon {i+1}', edgecolor='black')
    center = sum(poly, Vector2(0, 0)) * (1 / len(poly))
    ax.text(*center, f'$P_{i+1}$', color='black', fontsize=18, ha='center', va='center', zorder=6)

ax.plot(*zip(*path), color='blue', label='Path', linewidth=2, zorder=4)
for point in path[1:-1]:
    ax.scatter(*point, color='blue', edgecolor='white', s=50, zorder=5)

plt.savefig('i_path.png', dpi=300, bbox_inches='tight')
