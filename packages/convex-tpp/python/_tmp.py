...
import common
from common import Solution, locate_point_binary_search2
from test_solution import make_test

def test(seed: int) -> None:
	...
	random.seed(seed)
	start, target, polygons, _ = make_test([500, 100, 300], 1)
	...
	locations = [locate_point_binary_search2(v, polygon1, cones.__getitem__) for v in polygon2]
	fc = solution.first_contact[len(polygons) - 2]

	plt.plot(locations, "o-", color="blue")

	fig, axs = plt.subplots(2, 3, figsize=(6, 6), constrained_layout=True)

	drawing = Drawing(start, target, polygons)

	flat = axs.flatten() if len(axs) > 1 else [axs]
	...
	drawing.draw_scene(flat[0], -1)
	drawing.draw_scene(flat[1], len(polygons) - 2)
	
	for i, polygon in enumerate(polygons):
		center = sum(map(Vector2, polygon), Vector2(0, 0)) / len(polygon)
		flat[0].text(center.x, center.y, f"{i + 1}", ha='center', va='center')
	
	flat[0].legend()

	vertices = [v for v, loc in zip(polygon2, locations) if loc % 2 == 1 and (fc[loc // 2] or fc[(loc // 2 + 1) % len(polygon1)])]

	for i, polygon in enumerate(polygons):
		center = sum(map(Vector2, polygon), Vector2(0, 0)) / len(polygon)
		flat[2].text(center.x, center.y, f"{i + 1}", ha='center', va='center')
		flat[2].fill(*zip(*polygon), alpha=0.5, label=f'Polygon {i + 1}')

	if vertices:

		flat[2].scatter(*zip(*vertices), color="red", alpha=0.5)
		flat[2].fill(*zip(*vertices), color="red", alpha=0.5)

	reflected = []

	for v, loc in zip(polygon2, locations):

		if loc % 2 == 0 or (not fc[loc // 2] and not fc[(loc // 2 + 1) % len(polygon1)]):
			continue

		edge_index = loc // 2
		v1 = polygon1[edge_index]
		v2 = polygon1[(edge_index + 1) % len(polygon1)]

		r = common.vector_reflect_segment(v, v1, v2)
		reflected.append(r)
	
	if reflected:

		flat[3].scatter(*zip(*reflected), color="red", alpha=0.5)
		flat[3].fill(*zip(*reflected), color="red", alpha=0.5)

	for i, polygon in enumerate(polygons):
		center = sum(map(Vector2, polygon), Vector2(0, 0)) / len(polygon)
		# flat[3].text(center.x, center.y, f"{i + 1}", ha='center', va='center')
		flat[3].fill(*zip(*polygon), alpha=0.5, label=f'Polygon {i + 1}')

	if reflected:

		flat[4].scatter(*zip(*reflected), color="red", alpha=0.5)
		flat[4].fill(*zip(*reflected), color="red", alpha=0.5)

	drawing.draw_scene(flat[5], len(polygons) - 3)

	if reflected:

		flat[5].scatter(*zip(*reflected), color="red", alpha=0.5)
		flat[5].fill(*zip(*reflected), color="red", alpha=0.5)


	fig, ax = plt.subplots(figsize=(6, 6), constrained_layout=True)

	locations2 = [locate_point_binary_search2(v, polygons[-3], solution.cones[-3].__getitem__) for v in reflected]

	print(locations2)

	ax.plot(locations2, "o-", color="blue")
