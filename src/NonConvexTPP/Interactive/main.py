...
class Main:
	...
	def draw_polygon(self, polygon: Polygon) -> None:
		...
		# try decomposition, but timeout after 0.5 seconds to prevent freezing on very complex polygons


		parts = hertel_mehlhorn(p2) # type: ignore
	...
	def draw_solution(self) -> None:
		...
		if any(len(poly.vertices) < 3 for poly in self.polygons):
			return
	...
	def run(self) -> None:
		...
		self.place_polygon([(2, 3), (2, 2), (-1, 2), (-1, 3)], POINTS_COLORS[0])
