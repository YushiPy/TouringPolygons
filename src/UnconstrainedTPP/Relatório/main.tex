...
Consideremos um polígono $P_i$. Dado um vértice $v \in T_i$, a região $R(v) \in S_i$ é formada pelos pontos $p \in \mathbb{R}^2$ tais que o $(i-1)$-esimo caminho até $p$ passa por $v$. Dada uma aresta $e \in T_i$, definimos de forma análoga $R(e) \in S_i$ como a união dos pontos $p \in \mathbb{R}^2$ tais que o $(i-1)$-esimo caminho até $p$ passa por $e$. Definimos ainda $R_{\mathrm{passagem}} = \mathbb{R}^2 \setminus \cup_{v \in T_i} R(v) \setminus \cup_{e \in T_i} R(e)$. Dessa forma $R_{\mathrm{passagem}}$, $R(v)$ para todo $v \in T_i$ e $R(e)$ para todo $e \in T_i$ formam uma partição do plano.
...
Uma vez que construímos \(T_i\) e \(S_i\), o próximo passo é implementar a função \(\text{Consulta}\) que retorna o \(i\)-path até um ponto \(p\). No entanto, isso requer localizar o ponto \(p\) na partição \(S_i\), ou seja, determinar qual vértice, aresta ou região de travessia contém o ponto \(p\). Primeiramente, vamos descrever uma solução em tempo \(O(\abs{S_i})\), que itera sobre todas as regiões de vértice e aresta de \(S_i\) e verifica se o ponto \(p\) pertence a alguma dessas regiões. Em seguida, vamos descrever como otimizar essa solução para tempo \(O(\log(\abs{P_i}))\) usando uma estratéfia apropriada baseada em busca binária.

Para ambas soluções, precisamos conseguir verificar se um ponto \(p\) pertence a uma região de vértice ou a uma região de aresta específicos. Para uma região de vértice associada a um vértice \(v\) de \(T_i\), precisamos apenas verificar se o segmento \(\overline{vp}\) está contido no cone formado pelos vetores \(d^1\) e \(d^2\), calculados no Algoritmo~\algref{problem1-vertex-region}. Para tal, devemos considerar dois casos, dependendo do ângulo entre \(d^1\) e \(d^2\). Se o ângulo for menor ou igual a \(180 ^ \circ\), então \(p\) pertence à região de vértice se e somente se \(p\) está à esquerda de \(d^1\) \textbf{e} à direita de \(d^2\). Caso contrário, \(p\) pertence à região de vértice se e somente se \(p\) está à esquerda de \(d^1\) \textbf{ou} à direita de \(d^2\), as Figuras~\ref{fig:problem1-cone-check-small} e \ref{fig:problem1-cone-check-large} ilustram esses dois casos. Usando essas ideias, implementamos o procedimento \algref{PontoEmVértice} para verificar se um ponto \(p\) pertence a uma região de vértice específica.
...
\begin{figure}
	...
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Problem1Images/cone-check-small.png}
		\caption{Região de vértice onde \(d ^ 1 \times d ^ 2 > 0\).}
		\label{fig:problem1-cone-check-small}
	\end{minipage}
	\hfill
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{Problem1Images/cone-check-large.png}
		\caption{Região de vértice onde \(d ^ 1 \times d ^ 2 < 0\).}
		\label{fig:problem1-cone-check-large}
	\end{minipage}

\end{figure}

\begin{algorithm}
\alglabel{problem1-vertex-region-check}
\caption{PontoEmVértice}
\Input{\(p\), \(v\), \(d^1\), \(d^2\)}
\Output{\texttt{true} se \(p\) pertence à região de vértice associada a \(v\) delimitada por \(d^1\) e \(d^2\), \texttt{falso} caso contrário.}

\vspace{0.3em}
\hrule
\vspace{0.3em}

\If {\(d^1 \times d^2 \ge 0\)} {
	\Return {\(d ^ 1 \times (p - v) \ge 0 \land d ^ 2 \times (p - v) \le 0\)}
}
\Else {
	\Return {\(d ^ 1 \times (p - v) \ge 0 \lor d ^ 2 \times (p - v) \le 0\)}
}

\end{algorithm}

Ademais, também precisamos verificar se um ponto \(p\) pertence a uma região de aresta associada a uma aresta \((u, v)\) de \(T_i\) com direções \(d^1\) partindo de \(u\) e \(d^2\) partindo de \(v\). A princípio, temos apenas um único caso, pois pela maneira que determinamos as direções \(d^1\) e \(d^2\), é impossível que \(d ^ 1 \times d ^ 2 < 0\), dessa forma, temos um único caso para considerar, no qual, \(p\) pertence à região de aresta se e somente se \(p\) está entre as semi-retas definidas por \(d^1\) e \(d^2\) \textbf{e} \(p\) está do lado externo da aresta \((u, v)\). Essencialmente, estamos lidando com o caso da Figura~\ref{fig:problem1-cone-check-small} com a restrição adicional de que o ponto deve estar do lado correto da aresta. Usando essas ideias, implementamos o Algoritmo~\algref{problem1-edge-region-check} para verificar se um ponto \(p\) pertence a uma região de aresta específica.

\begin{algorithm}
\alglabel{problem1-edge-region-check}
\caption{PontoEmAresta}
\Input{\(p\), \(u\), \(v\), \(d^1\), \(d^2\)}
\Output{\texttt{true} se \(p\) pertence à região de aresta associada a \((u, v)\) delimitada por \(d^1\) e \(d^2\), \texttt{falso} caso contrário.}

\vspace{0.3em}
\hrule
\vspace{0.3em}

\Return {\(d ^ 1 \times (p - u) \ge 0 \land d ^ 2 \times (p - v) \le 0 \land ((v - u) \times (p - u) \le 0)\)}
\end{algorithm}

Finalmente, podemos usar os Algoritmos \algref{problem1-vertex-region-check} e \algref{problem1-edge-region-check} para localizar um ponto \(p\) na partição \(S_i\) iterando sobre todas as regiões de vértice e aresta de \(S_i\) e verificando se \(p\) pertence a alguma dessas regiões. Se \(p\) pertencer a uma região de vértice associada a um vértice \(v\), retornamos esse vértice. Se \(p\) pertencer a uma região de aresta associada a uma aresta \((u, v)\), retornamos essa aresta. Caso contrário, retornamos que \(p\) pertence à região de travessia. Implementamos esse procedimento no Algoritmo~\algref{problem1-locate-point-linear}. Vale notar que não detalhamos o que é retornado exatamente, mas isso pode ser facilmente implementado retornando o índice do vértice ou da aresta na estrutura de dados que representa \(S_i\).

\begin{algorithm}
\alglabel{problem1-locate-point-linear}
\caption{LocalizaPonto}
\Input{\(p\), \(T_i\), \(S_i\)}
\Output{Região de \(S_i\) que contém \(p\) (vértice, aresta ou travessia).}
...
\For {\(v\) vértice de \(T_i\)} {
	...
	\((d^1, d^2) \leftarrow\) direções associadas a \(v\) em \(S_i\) \\

	\If {\text{PontoEmVértice}(\(p\), \(v\), \(d^1\), \(d^2\))} {
		\Return {\(v\)}
	}
}

\For {\((u, v)\) aresta de \(T_i\)} {

	\(d^1 \leftarrow\) segunda direção associada a \(u\) em \(S_i\) \\
	\(d^2 \leftarrow\) primeira direção associada a \(v\) em \(S_i\) \\

	\If {\text{PontoEmAresta}(\(p\), \(u\), \(v\), \(d^1\), \(d^2\))} {
		\Return {\((u, v)\)}
	}
}

\Return {Região de travessia}

\end{algorithm}

\subsection{Localizando Pontos na Partição Eficientemente}

Na seção anterior, descrevemos uma maneira de localizar um ponto \(p\) na partição \(S_i\) em tempo \(O(\abs{S_i})\) iterando sobre todas as regiões de vértice e aresta de \(S_i\). No entanto, podemos otimizar essa solução para tempo \(O(\log(\abs{P_i}))\) usando uma estratégia baseada em busca binária. Para tal, não precisamos de nenhuma estrutura de dados adicional, apenas perceber que as regiões de vértice e aresta de \(S_i\) podem ser unidas em uma única região de aresta, permitindo que verifiquemos se \(p\) pertence à essa nova região em tempo \(O(1)\), permitindo assim o uso de busca binária.

Essencialmente, sejam \(v^1, \dots, v^n\) vértices consecutivos de \(T_i\) em sentido anti-horário. Para determinar se um ponto \(p\) pertence à \textbf{alguma} região de vértice desses vértices ou à alguma região de aresta entre esses vértices, poderíamos verificar cada região de vértice e aresta individualmente, como fizemos na seção anterior. No entanto, também podemos considerar a região de aresta definida pela semi-reta partindo de \(v^1\) até \(v^n\), com raios \(d^1\) e \(d^2\) onde \(d^1\) é o primeiro raio de \(v^1\) e \(d^2\) é o segundo raio de \(v^n\). Note que essa região de aresta contém todas as regiões de vértice e aresta entre \(v^1\) e \(v^n\) e não contém nenhuma outra região de vértice ou aresta. Assim, podemos verificar se \(p\) pertence a alguma região de vértice ou aresta entre \(v^1\) e \(v^n\) simplesmente verificando se \(p\) pertence a essa nova região de aresta. Usando essa ideia, podemos implementar uma busca binária para localizar \(p\) na partição \(S_i\).

A Figura~\ref{problem1-edge-regions} ilustra essa ideia, onde temos quatro regiões de vértice e três regiões de aresta unidas em uma única região de aresta maior. Note que essa nova região de aresta pode ser verificada usando o Algoritmo~\algref{problem1-edge-region-check}, permitindo assim a implementação da busca binária.

No entanto, também temos outros casos e infelizmente, o Algoritmo~\algref{problem1-edge-region-check} não é suficiente para verificar se um ponto \(p\) pertence a uma região de aresta arbitrária, como a que acabamos de definir, pois a união de regiões de vértice e aresta pode gerar diferentes casos para a região de aresta resultante, dependendo do ângulo entre \(d^1\) e \(d^2\). Por esse motivo, precisamos primeiro estender o Algoritmo~\algref{problem1-edge-region-check} para lidar com esses casos adicionais antes de podermos implementar a busca binária. 
...
\begin{figure}[h]
	...
	\begin{minipage}{0.7\linewidth}
		...
		\begin{subfigure}{0.45\textwidth}
			\centering
			\includegraphics[width=\linewidth]{Problem1Images/edge-neg-neg.png}
			\caption{Ambos raios no lado negativo.}
		\end{subfigure}
		\hfill
		\begin{subfigure}{0.45\textwidth}
			\centering
			\includegraphics[width=\linewidth]{Problem1Images/edge-neg-pos.png}
			\caption{Primeiro raio no lado negativo e segundo no positivo}
		\end{subfigure}

		\begin{subfigure}{0.45\textwidth}
			\centering
			\includegraphics[width=\linewidth]{Problem1Images/edge-pos-neg.png}
			\caption{Primeiro raio no lado positivo e segundo no negativo.}
		\end{subfigure}
		\hfill
		\begin{subfigure}{0.45\textwidth}
			\centering
			\includegraphics[width=\linewidth]{Problem1Images/edge-pos-pos.png}
			\caption{Ambos raios no lado positivo.}
		\end{subfigure}

    \caption{Possiveis casos na união de regiões de vértice e aresta.}
    \label{problem1-edge-regions}
	\end{minipage}

\end{figure}

Essencialmente existem 4 casos possíveis para uma região de aresta, dependendo do ângulo entre as direções \(d^1\) e \(d^2\). A Figura \ref{problem1-edge-regions} ilustra esses casos. Para sabermos com qual caso estamos lidando, definimos \(w = v - u\) e calculamos os produtos vetoriais \(c_1 = w \times d^1\) e \(c_2 = w \times d^2\). Agora, temos os seguintes casos:

\begin{itemize}

	\item \(c_1 \le 0 \land c_2 \le 0\): Esse é o caso em que temos \textbf{ambas arestas no lado negativo} (Caso (a)), que é o caso tradicional que aparece na partição \(S_i\).
	
	\item \(c_1 \le 0 \land c_2 \ge 0\): Esse é o caso em que temos \textbf{aresta anterior no lado negativo e aresta posterior no lado positivo} (Caso (b)). 
	
	\item \(c_1 \ge 0 \land c_2 \le 0\): Esse é o caso em que temos \textbf{aresta anterior no lado positivo e aresta posterior no lado negativo} (Caso (c)). 
	
	\item \(c_1 \ge 0 \land c_2 \ge 0\): Esse é o caso em que temos \textbf{ambas arestas no lado positivo}. 
\end{itemize}
...
\alglabel{problem1-edge-region-check2}
\caption{PontoEmAresta}
\Input{\(p\), \(u\), \(v\), \(d^1\), \(d^2\)}
\Output{\texttt{true} se \(p\) pertence à região de aresta associada a \((u, v)\) delimitada por \(d^1\) e \(d^2\), \texttt{falso} caso contrário.}

\If {\(\abs{u - v} < 10 ^ {-8}\)} {
	\Return {} PontoEmVértice(\(p\), \(v\), \(d^2\), \(d^1\))
}
...
\If {\((v - u) \times d ^ 1 < 0\)} {
	...
	\Else {
		\If {\((v - u) \times (p - u) < 0\)} {
			\Return {} \(d ^ 1 \times (p - u) \ge 0\)
		}
		\Else {
			\Return {} \(d ^ 2 \times (p - v) \le 0\)
		}
	}
}
\Else {
	...
	\Else {
		\Return {} \(d ^ 1 \times (p - u) > 0 \lor d ^ 2 \times (p - v) < 0 \lor (v - u) \times (p - u) < 0\)
	}
}
...
\Function{pertence(\(i\), \(j\))} {
	...
	\(d^1 \leftarrow\) getDireção(\(i\)) \\
	\(d^2 \leftarrow\) getDireção(\(j\)) \\

	\Return{} PontoEmAresta(\(p\), \(v_1\), \(v_2\), \(d^1\), \(d^2\))
}
...
\begin{itemize}
	...
	\item Região de Vértice: Seja \(R = (v, d^1, d^2)\). Nesse caso, o \(i\)-path até \(p\) deve passar pelo vértice \(v\), tocando o polígono \(P_i\). Assim, temos que \(\text{Query}(p, i) = v\).
	
	\item Região de Aresta: Seja \(R = (u, v, d^1, d^2)\). Nesse caso, o \(i\)-path até \(p\) deve passar por algum ponto \(q\) da aresta \(e = (u, v)\), tocando o polígono \(P_i\). Para calcular \(q\), primeiro determinamos \(q' = \text{Query}(p', i - 1)\), onde \(p'\) é a reflexão de \(p\) em relação à aresta \(e\). 
