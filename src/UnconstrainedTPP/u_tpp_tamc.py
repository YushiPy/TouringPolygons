...
def tpp_solve(start: tuple[float, float], target: tuple[float, float], polygons: Sequence[Sequence[tuple[float, float]]], *, simplify: bool = False) -> list[tuple[float, float]]:
	...
	def build_rays(i: int, j: int, last: Vector2) -> None:
		"""
		Builds the rays for vertex `j` of polygon `i`, given that the last step of the path to `vertex` is `last`.
		"""
	...
	def locate_point_linear_seach(point: Vector2, i: int) -> int:
		...
		for j in range(len(polygon)):
			...
			if point_in_edge(point, v1, v2, ray1, ray2):
	...
	def query(point: Vector2, i: int, location: int | None = None) -> Vector2:
		"""
