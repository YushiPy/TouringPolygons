...
class Game(ABC):
	...
	def __init__(self, __fps: int | float = 125) -> None:
		...
		self.__display = pg.display.set_mode((WIDTH, HEIGHT), pg.RESIZABLE)
	...
	def __loop(self, __start: int) -> int:
		...
		if self.update(*self.__all_events) is not None: 
			self.__run_game = False
	...
	def __manage_events(self) -> None:
		...
		if pg.QUIT in self.events: 
			self.__run_game = False
		
		if pg.K_ESCAPE in self.down_keys: 
			self.__run_game = False
