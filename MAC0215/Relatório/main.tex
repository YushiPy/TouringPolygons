...
Embora não tenhamos conseguido explorar essa variação muito profundamente ainda, a ideia central é tratar o problema como um problema deotimização inteira de uma função quadrática sujeita a restrições lineares. Dessa forma podemos usar técnicas de otimização matemática para otimizar a abordagem inicial, assim como utilizar solvers comerciais para comparar resultados e validar nossa implementação.
...
\begin{itemize}
	...
	\item \textbf{Redação do Relatório Final e Vídeo de Apresentação}: Investimos tempo significativo na redação do relatório final desse projeto, assim como na criação do vídeo de apresentação, incluindo a revisão, formatação e geração das figuras apresentadas.
	
	\item \textbf{Proposta para a Fapesp}: Investimos tempo na criação e submissão da proposta de projeto para a Fapesp, incluindo planejamento, redação, revisão e reuniões com o orientador para discutir e aprimorar a proposta, além do tempo para conseguir os documentos necessários para a submissão. Esse processo foi repetido para a reconsideração.
