...
Enquanto essa solução funciona corretamente, sua complexidade é \(O(\abs{S_i})\), que pode ser tão grande quanto \(O(\abs{P_i})\) no pior caso. No entanto, podemos otimizar essa solução para tempo \(O(\log(\abs{P_i}))\) usando uma estratégia baseada em busca binária. Ilustramos essa solução mais lenta para ilustrar os conceitos básicos e motivar uma solução mais rápida que descrevemos a seguir.
