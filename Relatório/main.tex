...
\begin{description}[labelsep=1em, leftmargin=3em]
	\item[$\mathbb{R}$] The set of real numbers.
	\item[$n$] The size of the input.
	\item[$f : \mathbb{R}^n \to \mathbb{R}$] A function mapping an $n$-dimensional vector to a real value.
	\item[$\nabla f(x)$] The gradient of $f$ at the point $x$.
	\item[$\|x\|$] The Euclidean norm of the vector $x$.
\end{description}
...
Primeiramente, definimos um \(i\)-path até \(p\) como um caminho mínimo que se inicia em \(s\) e toca os polígonos \(P_1, \ldots, P_i\) em ordem, terminando em um ponto \(p\). Assim, podemos definir a função \(\text{Query}(p, i)\) que retorna o ponto \(q\) que antecede \(p\) no \(i\)-path até \(p\). Note que se \(i = 0\), então \(\text{Query}(p, 0) = s\) para qualquer ponto \(p\).
