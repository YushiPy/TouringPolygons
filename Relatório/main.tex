...
\begin{wrapfigure}{r}{0.40\textwidth}
	...
	\includegraphics[width=0.4\textwidth]{Problem1Images/scene-edge-case.png}
	\caption{Partição de um polígono com uma única aresta não visíveis.}
	\label{problem1-scene-edge-case}
...
Usando essa função, definimos \(d_1 = \text{reflect}(d, (v_1, v))\) se a aresta \((v_1, v)\) for visível e \(d_1 = d\) caso contrário a mesma lógica seque para \(d_2\).
...
Isso completa a construção das regiões de vértice. No entanto, ainda não esclarecemos como determinar se uma aresta é visível ou não de maneira eficiente. % TODO
