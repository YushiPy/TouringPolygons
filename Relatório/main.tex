...
\]
é tal que $v \times u \geq 0$ se partindo de $v$ e andando no sentido anti-horario, o angulo com $u$ é de no maximo 180 graus e $v \times u \leq 0$ se partindo de $v$ e andando no sentido horario, o angulo com $u$ é de no maximo 180 graus.


Essa representação já permite que verifiquemos se um ponto \(p\) pertence a uma região de vértice e qual. \textcolor{red}{Deve aqui pelo menos dizer: Explicaremos isso mais adiante no Algoritmo~4.} No entanto, não conseguimos distinguir entre regiões de aresta e a região de atravessar. Para resolver esse problema, armazenamos uma lista de booleanas que indica se uma aresta é visível ou não. Assim, para determinar se um ponto \(p\) pertence a uma região de aresta ou à região de atravessar, basta verificar se \(p\) está entre as semi-retas de duas regiões de vértice adjacentes e, em caso positivo, verificar se a aresta que as separa é visível ou não.
