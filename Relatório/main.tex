...
\begin{minipage}{0.44\textwidth}
	\centering
	\includegraphics[width=\linewidth]{last_step_map_new.png}
	\caption{Nova interpretação do mapa de último passo \(S_1\) de um polígono \(P_1\). Representamos as regiões de vértice em vermelho e as de aresta em verde.}
	\label{fig:last_step_map-new}
\end{minipage}
\hspace{10pt}
\begin{minipage}{0.44\textwidth}
	\centering
	\includegraphics[width=\linewidth]{fake_edge.png}
	\caption{Criamos uma aresta fictícia entre os vértices \(v^3\) e \(v^6 = v^1\) de \(P_1\) e definimos a região \(R\), que engloba todas as regiões entre eles.}
	\label{fig:fake_edge}
\end{minipage}
\end{figure}
...
\begin{description}
	...
	\item[\(r^1\) e \(r^2\) externos:] \(c_1 \land c_2 \land c_3\). Veja a Figura~\ref{fig:binary_search_cases}(a).

	\item[\(r^1\) e \(r^2\) internos:] \(c_1 \lor c_2 \lor c_3\). Veja a Figura~\ref{fig:binary_search_cases}(b).

	\item[\(r^1\) externo e \(r^2\) interno:] \((c_1 \land c_3) \lor (c_2 \land \lnot c_3)\). Veja a Figura~\ref{fig:binary_search_cases}(c).
	
	\item[\(r^1\) interno e \(r^2\) externo:] \((c_1 \land \lnot c_3) \lor (c_2 \land c_3)\). Veja a Figura~\ref{fig:binary_search_cases}(d).
