...
\begin{itemize}
	...
	\item RefleteDireção: \(O(1)\), pois faz um número constante de operações.
	\item PontoEmVértice: \(O(1)\), pois faz um número constante de operações.
	\item PontoEmAresta: \(O(1)\), pois faz um número constante de operações e chamadas para PontoEmVértice.
	
	\item Query: \(O(\sum_{j = 1}^{i} \log(\abs{P_j}))\), pois faz no máximo \(i\) chamadas recursivas, cada uma levando \(O(\log(\abs{P_j}))\) para cada \(j\).

	\item ArestasVisíveis: \(\abs{P_i} \sum_{j = 1}^{i} \log(\abs{P_j})\), pois calculamos \(\abs{P_i}\) visibilidades, cada uma levando \(O(\sum_{j = 1}^{i} \log(\abs{P_j}))\) para chamar Query.

	\item LocalizaRegião: \(O(\log(\abs{P_i}))\), pois usa busca binária em \(2 \abs{P_i} - 1\) regiões.
	
	\item Particiona: \(O\left(\sum_{i = 1}^{k} i \abs{P_i} \log(\abs{P_i})\right)\), pois para cada polígono \(P_i\) chamamos ArestasVisíveis e iteramos sobre seus vértices fazendo chamadas para Query. As chamadas para Query tem a mesma complexidade que ArestasVisíveis.

	\item CaminhoMínimo: \(O\left(\sum_{i = 1}^{k} i \abs{P_i} \log(\abs{P_i})\right)\), pois chamamos Particiona uma vez e depois fazemos \(k\) chamadas para Query, cada uma levando \(O(i \log(\abs{P_i}))\).
