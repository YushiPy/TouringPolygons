...
\begin{blockQuote}
	\hypertarget{lemma:edge_in_first_contact}{\textbf{Aresta em Região de Primeiro Contato:}} Seja~\(e = \overline{uv}\) uma aresta de~\(P_i\)~\((1 \le i \le k)\),~\(p\) um ponto qualquer dessa aresta (incluindo as pontas) e~\(q\) o ponto que precede~\(p\) no~\((i - 1)\)-path até~\(p\). Temos que~\(e \in T_i\) se, e somente se,~\(q\) está do lado externo de~\(e\), ou seja,~\((v - u) \times (q - u) < 0\). Caso contrário,~\(e \notin T_i\).
\end{blockQuote}
...
\begin{blockQuote}
	\hypertarget{lemma:local_optimality_conditions}{\textbf{Condições de Otimalidade Local:}}
...
Uma vez que sabemos calcular~\(T_i\) e~\(S_i\), desejamos implementar o algoritmo \texttt{LocalizaPonto}, que localiza um ponto~\(p\) no mapa de último passo~\(S_i\). Na primeira abordagem, implementamos esse procedimento de forma direta, verificando inicialmente todas as regiões associadas a vértices, depois todas as regiões associadas a arestas e, caso o ponto não pertença a nenhuma dessas regiões, retornando a região de atravessar~\(P_i\). Para verificar se um ponto~\(p\) pertence a uma região associada a um vértice~\(v\) de~\(P_i\) com raios~\(r^1\) e~\(r^2\), primeiro determinamos se o ângulo de abertura entre~\(r^1\) e~\(r^2\) é menor ou maior que~\(180^\circ\). Se o ângulo for menor que~\(180^\circ\), então~\(p\) pertence à região associada a~\(v\) se o produto vetorial entre~\(r^1\) e~\((p - v)\) for positivo \textbf{e} o produto vetorial entre~\((p - v)\) e~\(r^2\) for negativo. Caso contrário,~\(p\) pertence à região associada a~\(v\) se o produto vetorial entre~\(r^1\) e~\((p - v)\) for positivo \textbf{ou} o produto vetorial entre~\((p - v)\) e~\(r^2\) for negativo. Essas situações são ilustradas na Figura~\ref{fig:point_in_cone}.
...
Uma vez que implementamos o algoritmo \texttt{LocalizaPonto} utilizando uma estratégia de busca binária, podemos substituir a implementação do Algoritmo~\algref{alg:locate_point_first_approach} pela implementação do Algoritmo~\algref{alg:locate_point_second_approach} no Algoritmo~\algref{alg:tpp_complete-1-2}, mantendo os algoritmos de cálculo de regiões de primeiro contato e mapas de último passo inalterados.
...
Essa abordagem irá reutilizar muitas das ideias apresentadas nas últimas duas abordagens. Primeiramente, observe que os algoritmos de consulta (Algoritmo~\algref{alg:consulta-caminho} e Algoritmo~\algref{alg:consulta-ultimo-passo}) não usam os valores de \(T_i\) e \(S_i\) explicitamente, apenas repassando-os para o algoritmo \texttt{LocalizaPonto}. Assim, podemos manter esses dois algoritmos inalterados. Infelizmente, não podemos reutilizar os algoritmos que calculam as regiões de primeiro contato e os mapas de último passo (Algoritmo~\algref{alg:first_contact_region} e Algoritmo~\algref{alg:last_step_map}) diretamente, pois eles calculam~\(T_i\) e~\(S_i\) por completo, o que não é eficiente.

Retomando as conclusões de \hyperlink{lemma:edge_in_first_contact}{\textbf{Aresta em Região de Primeiro Contato}} e \hyperlink{lemma:local_optimality_conditions}{\textbf{Condições de Otimalidade Local}}. Seja \(v\) um vértice de \(P_i\) com arestas incidentes \(e^1\) e \(e^2\) e \(q\) último passo do \((i - 1)\)-path até \(v\). Como \(v\) faz parte tanto de \(e^1\) e \(e^2\), temos que \(e^1 \in T_i\) se e somente se \(q\) está do lado externo de \(e^1\) e \(e^2 \in T_i\) se e somente se \(q\) está do lado externo de \(e^2\). Além disso, uma vez que sabemos se \(e^1\) e \(e^2\) pertencem a \(T_i\), podemos calcular os raios associados a \(v\) em \(S_i\) utilizando o algoritmo de reflexão apresentado no Algoritmo~\algref{alg:last_step_map}. 

A partir dessa ideia, segue que podemos determinar se uma aresta \(e\) pertence a \(T_i\) utilizando apenas uma consulta ao \((i - 1)\)-path até um dos seus vértices. Dessa forma, conseguimos calcular o cone de visibilidade de \(v\) em \(S_i\) e determinar se uma aresta \(e\) de \(P_i\) pertence a \(T_i\) usando apenas uma consulta. É importante mencionar que podemos guardar o resultado dessas consultas para evitar cálculos repetidos. Assim, calculamos os raios associados a um vértice de \(P_i\) e determinamos se uma aresta pertence a \(T_i\) uma única vez, e retornamos os resultados armazenados para as próximas consultas.

Dessa forma, podemos reutilizar a segunda implementação do algoritmo \texttt{LocalizaPonto} (Algoritmo~\algref{alg:locate_point_second_approach}). Para tal, deixamos implícito que as linhas 2 e 3 do Algoritmo~\algref{alg:locate_point_second_approach} que pedem os raios associados aos vértices \(u\) e \(v\) em \(S_i\) realizam as consultas necessárias para calcular os raios associados a \(u\) e \(v\) em \(S_i\), e armazenam os resultados para futuras consultas. Além disso, as linhas 28 e 30, que verificam se \(v^l \in T_i\) e se \(\overline{v^l v^r} \in T_i\), respectivamente, realizam as consultas necessárias para determinar se \(v^l\) e \(\overline{v^l v^r}\) pertencem a \(T_i\), e armazenam os resultados para futuras consultas.

Assim, concluimos a implementação da terceira abordagem do algoritmo \texttt{LocalizaPonto} e podemos finalizar a implementação completa do algoritmo TPP utilizando essa abordagem. Para tal, inicializamos \(S_1, \dots, S_k\) e \(T_1, \dots, T_k\) como conjuntos vazios e então retornamos o \(k\)-path até \(t\) utilizando o Algoritmo~\algref{alg:consulta-caminho}. Note que, nesse caso, o cálculo de \(T_i\) e \(S_i\) para cada \(i = 1, \dots, k\) é feito de forma implícita durante as consultas ao mapa de último passo \(S_i\), apenas para os vértices e arestas que são efetivamente necessários para localizar os pontos consultados em \(S_i\).
