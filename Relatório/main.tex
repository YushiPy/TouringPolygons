...
No entanto, precisamos de uma maneira eficiente de fazer isso para todas as arestas de \(P_i\). Se simplesmente fizermos isso de maneira ingênua, verificando se o segmento entre o último ponto \(q\) do \((i - 1)\)-path até o ponto médio \(m\) da aresta intersecta \(P_i\), teríamos uma complexidade de \(O(\abs{P_i})\) por aresta, resultando em \(O(\abs{P_i} ^ 2)\) para todas as arestas de \(P_i\). Felizmente, podemos fazer isso de maneira mais eficiente, explorando o fato de que \(P_i\) é convexo e suas arestas estão ordenadas em sentido anti-horário. 
...
No entanto, não ainda conseguimos distinguir entre regiões de aresta e a região de atravessar. Vamos resolver esse problema na seção seguinte.
...
\begin{enumerate}
	...
	\item \(v \in \mathbb{R} ^ 2 \implies v = (v_1, v_2)\). Vetores são representados como tuplas de coordenadas.
