...
\Input{\(s\), \((P_1, \dots, P_i)\), \((S_1, \dots, S_{i - 1})\), \((T_1, \dots, T_{i - 1})\)}
...
\For{\(v\) vértice de \(P_i\)} {
	...
	\(path \leftarrow \text{Query}(v, s, (P_1, \dots, P_{i - 1}), (T_1, \dots, T_{i - 1}), (S_1, \dots, S_{i - 1}))\) \\
...
\subsection{Regiões de Vértice}

Uma vez que determinamos \(T_i\), o próximo passo é contruir a partição \(S_i\) do plano. Conforme discutido acima, para representar \(S_i\), precisamos apenas contruir as regiões de v;vértice, pois as regiões de aresta podem ser derivadas das regiões de vértice adjacentes e a região de travessia é simplesmente o complemento das regiões de vértice e aresta. As 
