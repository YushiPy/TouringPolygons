...
class Gameplay(Game):
	...
	def draw(self, surface: pg.Surface) -> None:
		...
		#pg_image = pg.image.load("fullmetal.png").convert()
		#im_width, im_height = pg_image.get_size()

		#scale = min(surface.get_width() / im_width, surface.get_height() / im_height) * 0.8

		#pg_image = pg.transform.smoothscale(pg_image, (round(im_width * scale), round(im_height * scale)))

		# Draw with alpha blending
		#surface.blit(pg_image, (surface.get_width() // 2 - pg_image.get_width() // 2, surface.get_height() // 2 - pg_image.get_height() // 2))
