...
Dentre as diversas variações do problema, consideramos inicialmente uma versão simplificada em que os polígonos são convexos e não se sobrepõem. Consideramos ainda que a distância é medida na norma Euclidiana. Para esse caso, apresentamos três abordagens distintas. A primeira é uma implementação direta das ideias descritas em~\cite{tpp-dror2003}, baseadas em mapas de último passo. A segunda abordagem melhora a eficiência da primeira ao empregar uma estratégia de busca binária para a localização de pontos nesses mapas. A terceira abordagem utiliza uma técnica de memoização, com o objetivo de evitar cálculos redundantes e reduzir o custo computacional. Por fim, mostramos que a primeira abordagem possui complexidade~\(O(n^2)\), enquanto as outras duas apresentam complexidade~\(O(n k \log (n / k))\), onde~\(n\) é o número total de vértices de todos os polígonos e~\(k\) é o número de polígonos. \textcolor{blue}{Aqui falta uma frase explicando se as abordagens dois e três são mencionadas em~\cite{tpp-dror2003} ou não. Como está não fica claro se é algo novo que melhora o que foi apresentado em~\cite{tpp-dror2003} ou não. Eu entendo que a complexidade dessas duas abrdagens é sim a mencionada em~\cite{tpp-dror2003}. Logo, quiçá deva explicar alguma coisa.}

Em seguida, discutimos heurísticas para o caso geral do TPP, no qual os polígonos podem ser não convexos. Baseamo-nos principalmente na ideia de particionar polígonos não convexos num conjunto de polígonos convexos e aplicar, sobre eles, as soluções desenvolvidas para o caso convexo. \textcolor{blue}{Na intro e na frase anterior, fala de heurísticas. Mas se considerar todas as combinacoes, isso que descreve seria um método exato.} Além disso, exploramos técnicas de programação inteira mista para modelar o problema e empregar solvers comerciais, como o~\href{https://www.gurobi.com}{Gurobi}, na obtenção de soluções aproximadas. \textcolor{blue}{Se faz um modelo MILP e resolve com Gurobi, a priori, deveria encontrar um minimizador global verdadeiro. A menos que pare por tempo. Por que fala ``aproximadas''?} Por fim, comparamos os resultados obtidos por essas abordagens com aqueles produzidos pelas implementações propostas neste trabalho.
...
Todas as implementações a seguir têm como base as definições e propriedades apresentadas em~\cite{tpp-dror2003}. \textcolor{red}{Assumimos que o leitor possui familiaridade com o conteúdo desse artigo.} A seguir, incluímos as principais definições que serão utilizadas.
