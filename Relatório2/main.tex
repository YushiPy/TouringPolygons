...
\begin{blockQuote}
	...
	{\large \textbf{Problema 1: TPP Convexo}}
...
Todas as implementações a seguir têm como base as definições e propriedades apresentadas em~\cite{tpp-dror2003}. A seguir, incluímos as principais definições que serão utilizadas.

\begin{description}

	\item[Caminho Ótimo] (\(i\)-path): Um~\(i\)-path até um ponto~\(p \in \mathbb{R}^2\) é um caminho de comprimento mínimo que se inicia em~\(s\), termina em~\(p\) e visita os polígonos~\(P_1, P_2, \dots, P_i\) em ordem. É fácil notar que qualquer~\(i\)-path deve ser a união de segmentos de reta; assim, tratamos caminhos como sequências ordenadas de pontos em~\(\mathbb{R}^2\). 
	
	Veja a Figura~\ref{fig:exemplos-definições}(a) para um exemplo de um caminho mínimo que visita dois polígonos. Note que a ordem deve ser respeitada, pois visitar \(P_2\) antes de \(P_1\) seria mais eficiente nesse caso.
	
	\item[Região de Primeiro Contato] (\(T_i\)): Denotamos por~\(T_i\) à região de primeiro contato de um polígono~\(P_i\), definida como o conjunto de pontos~\(p\) em seu perímetro tais que o último segmento do~\((i - 1)\)-path até~\(p\) intersecta~\(P_i\) exclusivamente nesse ponto. Em outras palavras,~\(T_i\) consiste dos pontos $p$ no perímetro de~\(P_i\) tais que o último segmento do~\((i - 1)\)-path até $p$ não atravessa o interior de~\(P_i\).
	
	Veja a Figura~\ref{fig:exemplos-definições}(b) para um exemplo de região de primeiro contato de dois polígonos \(P_1\) e \(P_2\). Note que uma região de primeiro contato é sempre composta por uma região contínua de arestas do polígono, como é o caso de \(T_1\) e \(T_2\) na figura.

	\item[Mapa de Último Passo] (\(S_i\)): Denotamos por~\(S_i\) o mapa de último passo de um polígono~\(P_i\), que consiste em uma partição do plano em um número finito de regiões. Na partição podem existir duas ou mais regiões do tipo ``região de vértice'', uma ou mais regiões do tipo ``região de aresta'' e exatamente uma única região do tipo ``região de atravessar''. Cada região de vértice em $S_i$ está associada a um vértice $v$ de $P_i$ assim como cada região de aresta está associada a uma aresta $e$ de $P_i$. Isso não quer dizer que todo vértice e toda aresta de $P_i$ tenha uma região associada em $S_i$. Um ponto $p \in \mathbb{R}^2$ pertence a uma região de vértice associada ao vértice $v$ se e somente se o \(i\)-path até~\(p\) tem $\overline{vp}$ como último segmento. Analogamente,  um ponto $p \in \mathbb{R}^2$ pertence a uma região de aresta associada à aresta $e$ se e somente se o último segmento do \(i\)-path até~\(p\) inicia-se no interior de~$e$. Por último, um ponto $p \in \mathbb{R}^2$ pertence à região de atravessar se e somente se não pertence a nenhuma região de vértice ou aresta.
	
	Veja a Figura~\ref{fig:exemplos-definições}(c) para um exemplo de mapa de último passo de um polígono \(P_1\). As regiões de vértice são pintadas em vermelho, as regiões de aresta em verde e a região de atravessar em azul. Para cada região, incluimos um exmplo de ponto pertencente a ela, bem como o \(i\)-path até esse ponto, ilustrando a definição de cada tipo de região. Note que o mapa de último passo é uma partição do plano, ou seja, cada ponto do plano pertence a exatamente uma região do mapa, considerando que o polígono em si faz parte da região de atravessar.

\end{description}

\textcolor{blue}{Cada uma definicoes acima poderia terminar com a Frase "Veja a Figura X.". E uma figura poderia ilustrar o conceito.}

\begin{figure}[!htb]
	\centering

	% Top row: figures 1 and 2
	\begin{minipage}[t]{0.45\textwidth}
		\centering
		\includegraphics[width=\textwidth]{i_path.png}
		\caption*{(a) Caminho mínimo de \(s\) até \(t\) visitando \(P_1\) e \(P_2\).}
	\end{minipage}
	%\hfill
	\hspace{10pt}
	\begin{minipage}[t]{0.45\textwidth}
		\centering
		\includegraphics[width=\textwidth]{first_contact.png}
		\caption*{(b) Região de primeiro contato de \(P_1\) e \(P_2\).}
	\end{minipage}

	% Bottom figure (centered)
	\vspace{1em}
	\begin{minipage}[t]{0.45\textwidth}
		\centering
		\includegraphics[width=\textwidth]{last_step_map.png}
		\caption*{(c) Mapa de último passo de \(P_1\).}
	\end{minipage}

	\caption{Exemplos de \(i\)-path (a), região de primeiro contato (b) e mapa de último passo (c).}
	\label{fig:exemplos-definições}

\end{figure}

Como demonstrado em \cite[\textbf{Lemma 3}]{tpp-dror2003}, se dispomos de~\(S_1, \dots, S_i\), então é possível determinar o~\(i\)-path até qualquer ponto~\(p \in \mathbb{R}^2\) de maneira eficiente. Para tal, consideramos os três casos possíveis para o último segmento do~\(i\)-path até~\(p\), que dependem da região de~\(S_i\) à qual~\(p\) pertence:

\begin{itemize}

	\item \textbf{Região de Vértice}: Se~\(p\) pertence a uma região associada a um vértice~\(v\) de~\(P_i\), então o último segmento do~\(i\)-path até~\(p\) é o segmento~\(\overline{vp}\). Assim, o~\(i\)-path até~\(p\) é simplesmente o~\((i - 1)\)-path até~\(v\), seguido do segmento~\(\overline{vp}\). \textcolor{blue}{Aqui deve dizer: A Figura 1a ilustra esta situação.}
	
	\item \textbf{Região de Aresta}: Se~\(p\) pertence a uma região associada a uma aresta~\(e = \overline{v^1 v^2}\) de~\(P_i\), então refletimos o ponto~\(p\) em relação à reta que contém~\(e\), obtendo um ponto refletido~\(p'\). Calculamos o ~\((i - 1)\)-path até~\(p'\) cujo último segmento parte de um ponto~\(q'\), então chamamos de~\(q\) a interseção do segmento~\(\overline{q'p'}\) com a aresta~\(e\). O~\(i\)-path até~\(p\) é então formado pela parte do~\((i - 1)\)-path até~\(p'\) que vai desde o inicio (ponto $s$) até o ponto~\(q\), seguida do segmento~\(\overline{qp}\). \textcolor{blue}{Aqui deve dizer: A Figura 1b ilustra esta situação.}
	...
	\item \textbf{Região de Atravessar}: Se~\(p\) pertence à região de atravessar~\(P_i\), então o último segmento do~\(i\)-path até~\(p\) atravessa o interior de~\(P_i\). Nesse caso, o~\(i\)-path até~\(p\) é simplesmente o~\((i - 1)\)-path até~\(p\). \textcolor{blue}{Aqui deve dizer: A Figura 1c ilustra esta situação.}

\end{itemize}

A Figura~\ref{fig:casos_em_consulta} ilustra esses três casos. \textcolor{blue}{(Essa frase chegou tarde demais. Havia já muita coisa acumulada dificil de entender.} O caso~(a) mostra o ponto em uma região de vértice,~(b) ilustra o ponto em uma região de aresta, bem como o processo de reflexão e construção do caminho, e~(c) apresenta o ponto na região de travessia do polígono. 
...
Com base nessas ideias, implementamos o Algoritmo~\algref{alg:consulta-caminho}, que, dado um ponto~\(p\) e um índice~\(i\), retorna o~\(i\)-path até~\(p\) como uma sequência de pontos em~\(\mathbb{R}^2\), desconsiderando o último ponto do caminho,~\(p\), que é conhecido. Assumimos também a existência de uma função \texttt{LocalizaPonto}, que, \textcolor{blue}{dado $p \in \mathbb{R}^2$ e $1 \leq i \leq k$}, localiza o ponto~\(p\) no mapa de último passo~\(S_i\) de~\(P_i\) e retorna a região~\(R\) à qual~\(p\) pertence. Essa região carrega informações suficientes para determinar seu tipo (vértice, aresta ou atravessar) e os dados necessários para reconstruir o caminho conforme descrito anteriormente. \textcolor{blue}{Quiçá possa dizer aqui que informacao é essa. ``Anteriormente'' é muito vago. Num texto científico voce deve fazer referencia a uma formula ou coisa parecida.}
...
\Function {ConsultaCaminho(\(p, i, s, (P_1, \dots, P_i), (T_1, \dots, T_i), (S_1, \dots, S_i)\))} {
	...
	\ElseIf {\(R\) corresponde a uma aresta \(e = \overrightarrow{v^1 v^2}\) de \(P_i\)} {
		\(p' \leftarrow\) reflexão de \(p\) em relação à reta que contém \(e\) \ref{eq:reflection_on_edge} \\
		...
		\(q \leftarrow\) ponto de interseção entre as retas que passam por \(\overrightarrow{q' p'}\) e \(e\) \ref{eq:intersection_on_edge} \\
...
\Function {ConsultaÚltimoPasso(\(p, i, s, (P_1, \dots, P_i), (T_1, \dots, T_i), (S_1, \dots, S_i)\))} {
	...
	\ElseIf {\(R\) corresponde a uma aresta \(e\) de \(P_i\)} {
		...
		\(q \leftarrow\) ponto de interseção entre \(\overrightarrow{q' p'}\) e \(e\) \ref{eq:intersection_on_edge} \\
		\Return {} \(q\)
	}
	\ElseIf {\(R\) corresponde a atravessar \(P_i\)} {
...
Primeiramente, calculamos a região de primeiro contato~\(T_i\) de um polígono~\(P_i\). \textcolor{blue}{Aqui poderiamos dizer que, para um poligono $P_i$, $T_i$ é a primeira coisa que se calcula, antes de $S_i$. E que o calculo de $T_i$ requer o calculo previo de $T_j$ e $S_j$ para $j<i$.} Sabemos que essa \textcolor{blue}{(Se for colocar o que falie, o ``essa'' vai ficar longe e deve ser trocado por $T_i$)} região é a união de arestas de~\(P_i\), sendo delimitada por vértices de~\(P_i\)~\cite[\textbf{Lemma 2.}]{tpp-dror2003}. Assim, precisamos apenas determinar quais arestas de~\(P_i\) pertencem a~\(T_i\). Seja~\(e = \overline{v^1 v^2}\) uma aresta de~\(P_i\), \(p\) um ponto qualquer dessa aresta e~\(q\) o ponto que precede~\(p\) no~\((i - 1)\)-path até~\(p\). Note que~\(e \in T_i\) se, e somente se, a direção~\((q - p)\) está do lado externo de~\(P_i\). \textcolor{blue}{Essa frase esta estranha. Um segmento é uma coisa no plano. Uma direcao é um vetor que sai da origem a vai até um ponto no plano. Voce se refere a um segmento aqui?} Como o teste para determinar se uma aresta $e$ de $P_i$ pertence ou não a $T_i$ pode ser feito com um ponto $p$ qualquer em $e$, implementamos o teste considerando um vértice de $e$. A Figura~\ref{fig:first_contact_region} ilustra como podemos usar essa ideia para determinar a região de primeiro contato de um polígono, realizando o teste para ambos os vértices de cada aresta do polígono. Implementamos essa ideia no Algoritmo~\algref{alg:first_contact_region}, utilizando o vértice~\(v^1\) como o ponto~\(p\) para determinar se a aresta~\(e\) pertence a~\(T_i\).
...
\Function {LocalizaPonto(\(p, P_i, T_i, S_i\))} {
	...
	\For {aresta \(e = \overrightarrow{v^1 v^2}\) de \(P_i\) pertencente a \(T_i\)} {
...
A maior parte desse algoritmo é direta de implementar, exceto pela verificação se~\(p\) pertence à região~\(R_1\). Isso ocorre porque essa região pode assumir formas que não são cobertas pela verificação original do Algoritmo~\algref{alg:locate_point_first_approach}. Em particular, ao criar uma aresta fictícia entre dois vértices~\(v^1\) e~\(v^2\) com raios~\(r^1\) e~\(r^2\), pode ocorrer que esses raios formem um ângulo maior que~\(180^\circ\) ou estejam em lados distintos da aresta fictícia, como ilustrado na Figura~\ref{fig:binary_search_cases}.
...
\begin{enumerate}[label=(\alph*)]
	...
	\item \(d \times r^1 < 0 \land d \times r^2 < 0\): Verificamos se~\(p\) está entre os raios~\(r^1\) e~\(r^2\) e do lado externo da aresta~\(\overline{v^1 v^2}\).

	\item \(d \times r^1 > 0 \land d \times r^2 > 0\): Esse caso é o complemento do anterior. Verificamos se~\(p\) não está entre os raios~\(r^1\) e~\(r^2\) e se está do lado interno da aresta~\(\overline{v^1 v^2}\).

	\item \(d \times r^1 < 0 \land d \times r^2 > 0\): Inicialmente verificamos de que lado da reta que contém~\(\overline{v^1 v^2}\) o ponto~\(p\) se encontra. Se~\(p\) estiver do lado externo, verificamos se está do lado negativo de~\(r^2\); caso contrário, verificamos se está do lado positivo de~\(r^1\).
...
\Function {LocalizaPonto(\(p, P_i, T_i, S_i\))} {
	...
	\Return {} região associada a aresta \(\overrightarrow{u^{l} u^{r}}\)
}
