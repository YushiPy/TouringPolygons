...
Inicialmente, consideramos o TPP no caso em que os polígonos são convexos e não se sobrepõem. Sejam \(s, t \in \mathbb{R}^2\) pontos no plano e \(P_1, \dots, P_k\) uma sequência de polígonos convexos disjuntos, cada um descrito por uma sequência de vértices ordenados no sentido anti-horário. O objetivo é encontrar um caminho de comprimento mínimo que se inicia em \(s\), termina em \(t\) e visita cada polígono \(P_i\) ao menos uma vez, respeitando a ordem dada. Dizemos que um caminho visita os polígonos na ordem \(P_1, P_2, \dots, P_k\) se existem pontos \(p_1, p_2, \dots, p_k\) tais que \(p_i \in P_i\) para todo \(i = 1, 2, \dots, k\), e o caminho passa por esses pontos nessa mesma ordem. 

Neste trabalho, implementamos as ideias apresentadas por Dror et al. (2003) \cite{tpp-dror2003} para resolver esse problema de forma exata, assumindo familiaridade do leitor com as definições e resultados fundamentais do referido artigo. O código completo das implementações está disponível no \href{https://github.com/YushiPy/TouringPolygons}{repositório GitHub}.

Retomando as definições introduzidas por Dror et al. \cite{tpp-dror2003}, chamamos de \(i\)-path até um ponto \(p\) um caminho de comprimento mínimo que se inicia em \(s\), termina em \(p\) e visita os polígonos \(P_1, P_2, \dots, P_i\) em ordem. Conforme demonstrado no artigo original \cite[\textbf{Lemma 1.}]{tpp-dror2003}, todo \(i\)-path é único e pode ser representado como a união de segmentos de reta. Assim, ao longo deste trabalho, tratamos caminhos como sequências ordenadas de pontos em \(\mathbb{R}^2\).

A região de primeiro contato \(T_i\) de um polígono \(P_i\) é o conjunto de pontos \(p\) em seu perímetro tais que o último segmento do \(i\)-path até \(p\) intersecta \(P_i\) exclusivamente nesse ponto. Intuitivamente, \(T_i\) representa os pontos onde um caminho ótimo toca o polígono \(P_i\) pela primeira vez.

O mapa de último passo \(S_i\) associado ao polígono \(P_i\) é uma partição do plano em regiões tais que, para qualquer ponto \(p\) pertencente a uma mesma região, o último segmento do \(i\)-path até \(p\) possui a mesma origem: um vértice de \(P_i\), uma aresta de \(P_i\), ou atravessa o interior de \(P_i\).

Como demonstrado em \cite[\textbf{Lemma 3}]{tpp-dror2003}, uma vez conhecidas as regiões de primeiro contato \(T_0, \dots, T_i\), os mapas de último passo \(S_0, \dots, S_i\) e um procedimento capaz de localizar um ponto \(p\) em \(S_i\), é possível reconstruir o \(i\)-path até \(p\) de forma eficiente. A construção do caminho depende unicamente da região \(R \in S_i\) à qual o ponto \(p\) pertence.

Se a região \(R\) está associada a um vértice \(v\) de \(P_i\), então o último segmento do \(i\)-path até \(p\) é simplesmente o segmento \(\overline{vp}\), e o restante do caminho corresponde ao \((i-1)\)-path até \(v\). Se, por outro lado, \(R\) está associada a uma aresta \(e\) de \(P_i\), procede-se refletindo o ponto \(p\) em relação à reta que contém \(e\), obtendo-se um ponto \(p'\). Calcula-se então o \((i-1)\)-path até \(p'\), considerando-o apenas até o ponto em que seu último segmento intersecta a aresta \(e\), e conecta-se esse ponto a \(p\), formando assim o \(i\)-path. Finalmente, se a região \(R\) corresponde ao caso em que o caminho atravessa o interior de \(P_i\), o \(i\)-path até \(p\) coincide com o \((i-1)\)-path até \(p\).

\begin{figure}[!ht]
	\centering
	\includegraphics[width=0.99\textwidth]{Images/solutions.png}
	\caption{Três casos possíveis para o último segmento do \(i\)-path até um ponto \(p\).}
	\label{fig:path_query}
\end{figure}

A Figura~\ref{fig:path_query} ilustra os três casos possíveis para o último segmento do \(i\)-path até um ponto \(p\). O primeiro quadro mostra o caso em que o ponto está em uma região de vértice, o segundo quadro ilustra o caso em que o ponto está em uma região de aresta, assim como o processo de reflexão e construção do caminho, e o terceiro quadro apresenta o caso em que o ponto está na região de atravessar o polígono. 

Com base nessas ideias, implementamos o Algoritmo~\algref{alg:consulta-caminho}, que dado um ponto \(p\) e um índice \(i\), retorna o \(i\)-path até \(p\) como uma sequência de pontos em \(\mathbb{R}^2\) excluindo o último ponto do caminho \(p\), que é conhecido. Também assumimos que temos uma função \texttt{LocalizaPonto} que localiza o ponto \(p\) no mapa de último passo \(S_i\) e retorna a região \(R\) à qual \(p\) pertence, essa região carrega informações suficientes para determinar o tipo de região (vértice, aresta ou atravessar) e os dados necessários para reconstruir o caminho conforme descrito anteriormente.
