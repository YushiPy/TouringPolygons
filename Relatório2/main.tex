...
\begin{figure}[!ht]
	...
	\caption{Determinando a região de primeiro contato de um polígono.}
...
\Function {RegiãoDePrimeiroContato(\(i, s, (P_1, \dots, P_i), (T_1, \dots, T_{i - 1}), (S_1, \dots, S_{i - 1})\))} {
	...
	\For {cada aresta \(e\) de \(P_i\)} {
		...
		\If {\((q - v^1) \times (v^2 - v^1) < 0\)\footnotemark} {
...
\end{algorithm}
\footnotetext{Definimos o operador \(\times\) como o produto vetorial em \(\mathbb{R}^2\). Ou seja, para dois vetores \(a = (a_x, a_y)\) e \(b = (b_x, b_y)\), temos \(a \times b = a_x b_y - a_y b_x\).}
