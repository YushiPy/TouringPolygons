...
\For{vértice \(v\) em \(T_i\)} {
	...
	\If {\(e^1\) pertence a \(T_i\)} {
		\(r^1 \leftarrow\) reflexão de \(\overrightarrow{qv}\) em relação à reta que contém \(e^1\)
	}
	\Else {
		\(r^1 \leftarrow \overrightarrow{qv}\)
	}

	\If {\(e^2\) pertence a \(T_i\)} {
		\(r^2 \leftarrow\) reflexão de \(\overrightarrow{qv}\) em relação à reta que contém \(e^2\)
	}
	\Else {
		\(r^2 \leftarrow \overrightarrow{qv}\)
	}
...
\begin{figure}[H]
	...
	\label{fig:point_location_vertex}
\end{figure}

\begin{algorithm}
\alglabel{alg:point_location}
\caption{Localiza Ponto no Mapa de Último Passo}
\Input{\(p\), \(v\), \(r^1\), \(r^2\)}
