...
// polygonClipping is now a global, we use it directly.
 ...
 */
function removeHoles(polygon, holes, doNotCheckOrdering = false) {
 ...
 */
function _convexPartition(polygon, doNotCheckOrdering = false) {
...
/**
 * Converts polygon-clipping's [x, y] format to {x, y} objects.
 */
...
/**
 * Resolves self-intersections and partitions into convex polygons.
 * Accepts a polygon as {x, y}[] (any winding order).
 * Returns an array of convex polygons as {x, y}[][].
 */
export function convexPartition(polygon, holes = []) {
	const toRing = pts => [...pts.map(p => [p.x, p.y]), [pts[0].x, pts[0].y]];

	const outerRing = toRing(polygon);
	const holeRings = holes.map(toRing);

	// Union resolves self-intersections and produces valid simple polygons.
	const result = polygonClipping.union([outerRing, ...holeRings]);

	return result.flatMap(multiPoly =>
		multiPoly.flatMap((ring, i) => {
			const pts = toPoints(ring);
			// polygon-clipping: outer rings are CCW, holes are CW — matches poly-partition's convention.
			if (pts.length < 3) return [];
			try {
				return _convexPartition(pts, true); // doNotCheckOrdering=true, already correct winding
			} catch {
				return [pts]; // fallback: return as-is if partition fails
			}
		})
	);
}
