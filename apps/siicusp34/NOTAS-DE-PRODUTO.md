...
O roteiro apresenta seis cartões. O primeiro, da rota da USP, já aparece como
visitado e leva diretamente de volta ao desenho; os outros cinco abrem as
...
`benchmarks/suites/usp-butanta-50`, mais o novo contorno do Google (fid 94) na
camada `qgis/predios.gpkg` referenciada por `predios-usp.qgz`. O autor confirmou que `ime-a`, `ime-b` e
...
Na apresentação atual, a seleção USP / Caso 04 / Caso 10 / Explorar fica
imediatamente acima do desenho. A rota da USP usa a duração original da animação (cerca de 2,6 segundos a 1×); os controles de velocidade e a barra de
progresso continuam disponíveis. A seleção Explorar abre uma tabela dos 558
casos. No desktop, o desenho fica centralizado e ocupa até 70% da largura da
janela; no celular, usa a largura disponível. A faixa superior identifica o
TPP, o SIICUSP34 e o título do trabalho. O tempo de
resolução aparece em uma linha compacta. A
decomposição convexa ótima vem do pacote C++ do solver, é exportada na build e
fica desligada por padrão, mas pode ser ativada. A camada tem 458 peças para
mais de 50 regiões. Um registro antigo `google` (fid 48) continua sem geometria;
o exportador exige a versão desenhada (fid 94).
...
A página apresenta o caminho mínimo sem exibir jargão de gap numérico; as
tolerâncias usadas pelo solver estão registradas na proveniência técnica.
...
Os desenhos dos desafios vêm da campanha local `benchmarks/campaigns/SIICUSP34 - Instances/manual-cases.json` e são exportados por `scripts/build_challenge_data.mjs`, sem alterar o corpus de 558 casos. O script calcula 120 ordens, 16 escolhas de peças e 3.840 combinações para a versão atual dos desenhos. Os ótimos de ordem livre dos desafios 1 e 3 também foram conferidos com o solver C++, após desfazer a escala uniforme de exibição. Há alternativas próximas do ótimo. Cada desenho conserva suas coordenadas de exibição; os comprimentos são multiplicados por uma escala própria para que o melhor caminho de cada desafio valha 100 m. Esse metro é definido pelo modelo do desafio, não por medição física do campus. A interface mostra a comparação depois da tentativa e permite tocar diretamente nos polígonos e peças, também com teclado.
...
- `data/usp-demo.js` (~136 kB): mais de 50 regiões do campus e suas decomposições,
  fora do corpus;
- `data/challenge-data.js` (~416 kB): três desafios desenhados para o evento;
...
inicial ainda é de aproximadamente 16,8 MB. A página funciona com uma pasta
...
1. Conferir os nomes ainda provisórios em `polygons.csv`, os mais de 50 contornos e a
