  ...
  return `
    <span class="auto-sticker" tabindex="0" data-tooltip="${escapeHTML(label)}" aria-label="${escapeHTML(label)}">
      <svg viewBox="0 0 16 16" aria-hidden="true">
        <path d="M8 2.2l1.1 3.3h3.5L9.8 7.6l1.1 3.4L8 8.9 5.1 11l1.1-3.4-2.8-2.1h3.5L8 2.2z"></path>
      </svg>
    </span>
  `;
}
...
function loadEditorKeybinds() {
  try {
    ...
    return {
      closePolygon: Array.isArray(loaded.closePolygon)
        ? loaded.closePolygon.filter(Boolean)
        : loaded.closePolygon
          ? [loaded.closePolygon]
          : [...defaultKeybinds.closePolygon],
    };
  } catch {
    return { closePolygon: [...defaultKeybinds.closePolygon] };
  }
}
...
function updateKeybindUI() {
  const root = $("#close-polygon-keybinds");
  if (!root) {
    return;
  }
  root.innerHTML = "";
  editorKeybinds.closePolygon.forEach((binding, index) => {
    const wrapper = document.createElement("span");
    wrapper.className = "keybind-chip";
    const button = document.createElement("button");
    button.type = "button";
    button.className = "secondary keybind-input";
    button.textContent = pendingKeybindAction?.action === "closePolygon" && pendingKeybindAction.index === index ? "Press keys..." : binding;
    button.addEventListener("click", () => {
      pendingKeybindAction = { action: "closePolygon", index };
      updateKeybindUI();
    });
    ...
    remove.addEventListener("click", () => {
      editorKeybinds.closePolygon.splice(index, 1);
      saveEditorKeybinds();
      updateKeybindUI();
    });
  ...
  add.textContent = pendingKeybindAction?.action === "closePolygon" && pendingKeybindAction.index === editorKeybinds.closePolygon.length ? "Press keys..." : "...";
  add.addEventListener("click", () => {
    pendingKeybindAction = { action: "closePolygon", index: editorKeybinds.closePolygon.length };
    updateKeybindUI();
  });
...
const manualEditor = {
  ...
  frameCurrentCase() {
    ...
    this.scale = Math.max(0.1, Math.min(50000, Math.min(
      (width - 2 * pad) / spanX,
      (height - 2 * pad) / spanY,
    )));
  ...
  zoomBy(factor) {
    ...
    this.scale = Math.max(0.1, Math.min(50000, this.scale * factor));
  ...
  onPointerDown(event) {
    ...
    this.selectedPoints = this.activePoint?.kind === "vertex" ? [this.activePoint] : [];
    if (this.activePoint) {
  ...
  onPointerMove(event) {
    ...
    if (this.selectionRect) {
      ...
      this.selectedPoints = this.pointsInRect(this.selectionRect);
      this.draw();
      return;
    }
    if (this.panDrag) {
  ...
  deleteSelection() {
    const current = this.currentCase();
    const selected = this.selectedPoints.length > 0 ? this.selectedPoints : (this.selectedPoint ? [this.selectedPoint] : []);
