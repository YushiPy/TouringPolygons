...
function switchPanel(panelId) {
	...
	if (panelId === "benchmark-panel") {
		...
		refreshBenchmarkReport(state.selectedCampaign).catch((error) => setOutput($("#run-output"), error.message));
	}
	if (panelId === "comparison-panel") {
		refreshComparisonReport(state.selectedComparisonCampaign).catch((error) => setOutput($("#compare-output"), error.message));
	}
	if (panelId === "inspect-panel") {
		renderCampaigns();
		loadResults().catch((error) => setOutput($("#inspect-output"), error.message));
	}
	window.history.replaceState(null, "", `#${panelId.replace("-panel", "")}`);
...
function renderCampaignOptions({ loadReports = false } = {}) {
	...
	selectRunCampaign(nextSelected, { resetCap: nextSelected !== state.selectedCampaign, loadReport: loadReports });
	selectComparisonCampaign(state.campaigns.some((campaign) => campaign.name === state.selectedComparisonCampaign) ? state.selectedComparisonCampaign : fallback, { loadReport: loadReports });
	renderManualCampaigns();
...
function selectRunCampaign(name, options = {}) {
	...
	const loadReport = options.loadReport ?? true;
...
function renderResults(files) {
	...
	const visibleLimit = state.resultsExpanded ? 80 : 12;
	for (const file of filtered.slice(0, visibleLimit)) {
		const row = document.createElement("div");
		row.className = "result-row";
		const slash = file.path.lastIndexOf("/");
		const directory = slash >= 0 ? file.path.slice(0, slash + 1) : "";
		const name = slash >= 0 ? file.path.slice(slash + 1) : file.path;
		row.innerHTML = `<span>${escapeHTML(directory)}</span><strong>${escapeHTML(name)}</strong><small>${shortNumber(file.size)} B</small>`;
		root.appendChild(row);
	}
	const more = $("#toggle-results-button");
	if (more) {
		more.classList.toggle("is-hidden", filtered.length <= 12);
		more.textContent = state.resultsExpanded ? "Show fewer" : `Show all (${Math.min(filtered.length, 80)})`;
	}
}
...
function renderRunProgressCard(campaign, running = false, liveProgress = null) {
	...
	if (!root.querySelector("[data-run-progress-title]")) root.innerHTML = `
    <div class="run-progress-header">
      <strong data-run-progress-title></strong>
      <span data-run-progress-label></span>
    </div>
    <div class="run-progress-meta">
      <span data-run-progress-clock-label></span>
      <strong data-run-progress-clock></strong>
    </div>
    <div class="run-progress-line">
      <div class="run-progress-track">
        <div class="run-progress-fill"></div>
      </div>
      <strong class="run-progress-percent" data-run-progress-percent></strong>
    </div>
  `;
	root.querySelector("[data-run-progress-title]").textContent = title;
	root.querySelector("[data-run-progress-label]").textContent = label;
	root.querySelector("[data-run-progress-clock-label]").textContent = compiling ? "Compile wall clock" : running ? "Test wall clock" : "Wall clock";
	root.querySelector("[data-run-progress-clock]").textContent = formatElapsed(progress.elapsed_seconds || 0);
	root.querySelector("[data-run-progress-percent]").textContent = compiling ? "build" : `${percent}%`;
	const track = root.querySelector(".run-progress-track");
	track.classList.toggle("is-running", running);
	track.classList.toggle("is-compiling", compiling);
	track.querySelector(".run-progress-fill").style.width = `${visiblePercent}%`;
	root.classList.remove("is-hidden");
}
...
async function loadResults(force = false) {
	if (state.resultsLoaded && !force) {
		renderResults(state.resultFiles);
		return;
	}
	const resultData = await requestJSON("/api/results");
...
async function refreshExperimentState() {
	...
	const [campaignData, jobData] = await Promise.all([
...
async function refresh({ includeResults = state.activePanel === "inspect-panel" } = {}) {
	...
	try {
		...
		state.recentJobs = jobData.jobs;
		renderJobDock(previousJobs);
		renderCampaignOptions({ loadReports: false });
		if (state.activePanel === "inspect-panel") renderCampaigns();
		if (includeResults) await loadResults(true);
...
function renderComparisonProgress(progress, options) {
	...
	if (!progress.querySelector("[data-comparison-status]")) progress.innerHTML = `
    <div class="comparison-progress-head">
      <strong data-comparison-status></strong>
      <span>Test case: <strong data-comparison-case></strong></span>
      <span>Wall clock: <strong data-comparison-clock></strong></span>
      <span>Current solver: <strong data-comparison-solver></strong></span>
    </div>
    <div class="comparison-progress-lines">
      <div class="comparison-progress-line">
        <span>Instances Solved</span>
        <div class="run-progress-track" data-comparison-instances-track>
          <div class="run-progress-fill"></div>
        </div>
        <strong data-comparison-instances-value></strong>
      </div>
      <div class="comparison-progress-line">
        <span>Solvers Tested</span>
        <div class="run-progress-track" data-comparison-solvers-track>
          <div class="run-progress-fill"></div>
        </div>
        <strong data-comparison-solvers-value></strong>
      </div>
    </div>
  `;
	progress.querySelector("[data-comparison-status]").textContent = `Status: ${options.status}`;
	progress.querySelector("[data-comparison-case]").textContent = options.testCase || "-";
	progress.querySelector("[data-comparison-clock]").textContent = formatElapsed(options.elapsedSeconds || 0);
	progress.querySelector("[data-comparison-solver]").textContent = options.currentSolver || "-";
	progress.querySelector("[data-comparison-instances-value]").textContent = `${instancePercent}%`;
	progress.querySelector("[data-comparison-solvers-value]").textContent = options.solverTotal ? `${options.solverCompleted}/${options.solverTotal}` : "-";
	const instanceTrack = progress.querySelector("[data-comparison-instances-track]");
	const solverTrack = progress.querySelector("[data-comparison-solvers-track]");
	instanceTrack.classList.toggle("is-running", active);
	solverTrack.classList.toggle("is-running", active);
	instanceTrack.querySelector(".run-progress-fill").style.width = `${visibleInstancePercent}%`;
	solverTrack.querySelector(".run-progress-fill").style.width = `${visibleSolverPercent}%`;
...
async function pollJob(jobId) {
	...
	while (true) {
		let job;
		try {
			job = await requestJSON(`/api/jobs/${jobId}/progress`, { timeoutMs: 10_000 });
			requestFailures = 0;
		} catch (error) {
			requestFailures += 1;
			setOutput(output, `Connection interrupted (${requestFailures}/5). The run is still managed by the server.\n${error.message}`);
			if (requestFailures >= 5) throw error;
		...
		const liveProgress = job.phase === "benchmark" || job.progress_total
			? {
				completed: progressCompleted,
				total: job.progress_total,
				ratio: completedRun ? 1 : progressCompleted / job.progress_total,
				label: `${progressCompleted}/${job.progress_total || "?"} instances${job.current_item ? ` · ${job.current_item}` : ""}`,
			...
			: {
				...
				total: 0,
				ratio: 0,
				label: job.build_total ? `${job.build_completed || 0}/${job.build_total} build steps` : "Preparing build",
				elapsed_seconds: job.elapsed_seconds,
				counts: campaign?.run_index?.counts || {},
				phase: job.phase === "starting" ? "compile" : job.phase,
			};
...
async function pollComparisonJob(jobId) {
	...
	while (true) {
		...
		try {
			...
			requestFailures += 1;
			setOutput(output, `Connection interrupted (${requestFailures}/5). The comparison is still managed by the server.\n${error.message}`);
			if (requestFailures >= 5) throw error;
			await new Promise((resolve) => setTimeout(resolve, JOB_POLL_INTERVAL_MS * 2));
			continue;
		}
...
function resumeRunningJobs() {
	const active = state.recentJobs.filter((job) => job.status === "running" || job.status === "stopping");
	const run = active.find((job) => job.kind === "run");
	const comparison = active.find((job) => job.kind === "comparison");
	if (run && !state.currentRunJob) {
		state.currentRunJob = run.id;
		pollJob(run.id).catch((error) => setOutput($("#run-output"), `Could not reconnect to the run.\n${error.message}`));
	}
	if (comparison && !state.currentComparisonJob) {
		state.currentComparisonJob = comparison.id;
		pollComparisonJob(comparison.id).catch((error) => setOutput($("#compare-output"), `Could not reconnect to the comparison.\n${error.message}`));
	}
}
...
$("#toggle-results-button")?.addEventListener("click", () => {
...
$("#manual-background-opacity").addEventListener("input", (event) => {
	...
	input.style.setProperty("--opacity-progress", `${input.value}%`);
	manualEditor.setBackgroundOpacity(Number(input.value) / 100);
});
...
function syncBackgroundControls() {
	...
	if (background) {
		const opacity = $("#manual-background-opacity");
		opacity.value = String(Math.round((background.opacity ?? 0.45) * 100));
		opacity.style.setProperty("--opacity-progress", `${opacity.value}%`);
	}
}
