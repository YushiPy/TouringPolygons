...
import { THEME_STORAGE_KEY, UNSAFE_DELETION_STORAGE_KEY } from "./storage.js";
...
function applyUnsafeDeletion(enabled) {
	state.unsafeDeletion = enabled;
	localStorage.setItem(UNSAFE_DELETION_STORAGE_KEY, enabled ? "1" : "");
	const button = $("#unsafe-delete-toggle");
	if (!button) {
		return;
	}
	button.classList.toggle("is-active", enabled);
	button.setAttribute("aria-pressed", enabled ? "true" : "false");
	button.textContent = enabled ? "Unsafe Delete" : "Safe Delete";
}

function toggleUnsafeDeletion() {
	applyUnsafeDeletion(!state.unsafeDeletion);
}
...
async function deleteCampaign(event) {
	...
	if (!name || (!state.unsafeDeletion && !(await askConfirmation(`Delete campaign "${name}"? This removes its inputs, previews, results, and metadata.`, "Delete")))) {
		return;
	}
	try {
...
$("#unsafe-delete-toggle").addEventListener("click", toggleUnsafeDeletion);
...
applyUnsafeDeletion(localStorage.getItem(UNSAFE_DELETION_STORAGE_KEY) === "1");
manualEditor.init();
updateCreateMode();
