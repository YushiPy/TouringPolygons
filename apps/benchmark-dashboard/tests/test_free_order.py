...
class FreeOrderTests(unittest.TestCase):
    ...
    def test_free_command_uses_dedicated_runner_and_limits(self):
        request = RunCampaignRequest(name="sample", visit_order="free", solver="unordered", threads=1,
                                     max_calls="10", max_seconds="2", no_build=True, force=True)
    ...
    def test_rejects_wrong_solver_and_invalid_limits(self):
        for values in ({"solver": "binary"}, {"max_seconds": "nan"}, {"max_calls": "1,2"}, {"threads": 2}):
            with self.subTest(values=values), self.assertRaises(HTTPException):
                free_command(RunCampaignRequest(name="sample", visit_order="free", **values), Path("/tmp/a"), Path("/tmp/tpp.py"))
        with self.assertRaises(HTTPException):
            free_command(CompareSolversRequest(name="a", solvers=["unordered", "tspn"], max_seconds="0.5"),
                         Path("/tmp/a"), Path("/tmp/tpp.py"), comparison=True)
    ...
    def test_editor_dispatches_free_order_without_fixed_solver(self):
        ...
        with patch("dashboard.dashboard_free_order.solve_free_editor", fake_solve), patch.object(main, "ensure_live_solver_binary", side_effect=AssertionError("fixed solver called")):
            result = asyncio.run(solve(LiveSolveRequest(visit_order="free", target=(2, 0), polygons=[[(1, -1), (2, -1), (1, 1)]])))
    ...
    def test_free_api_builds_job_command(self):
        ...
        async def run():
            return await main.run_campaign(RunCampaignRequest(name="sample", visit_order="free", solver="unordered", threads=1, max_seconds="2"))

        with patch.object(main, "ensure_manual_binary_cache"), patch.object(main, "campaign_path", return_value=Path("/tmp/sample")), patch.object(main, "active_job", return_value=None), patch.object(main, "persist_jobs"), patch.object(main, "run_job", noop), patch.dict(main.jobs, clear=True):
