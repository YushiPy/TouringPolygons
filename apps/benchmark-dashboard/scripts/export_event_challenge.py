...
def solve(start: list[int], target: list[int], polygons: list) -> dict:
    ...
    process = subprocess.run(
        [main.SOLVER_BINARY],
        input=main.live_solver_input((start, target, polygons), 200000, 3),
        text=True,
        capture_output=True,
        check=True,
        timeout=8,
...
def piece_challenge() -> dict:
    ...
    pieces = [
        [[[x + a, y + b], [x + c, y + b], [x + c, y + d], [x + a, y + d]] for a, b, c, d in rectangles]
        for x, y in origins
    ]
    solutions = [
        {"choices": choice, **solve(start, target, [pieces[index][piece] for index, piece in enumerate(choice)])}
    ...
    return {
        ...
        "pieces": pieces,
        "solutions": solutions,
        "reference": min(range(27), key=lambda index: solutions[index]["length"]),
    }
            ...
            solutions.append(
                {
                    "order": order,
                    "choices": choices,
                    **solve(start, target, [pieces[index][choices[index]] for index in order]),
                }
            )
    return {
        "geometry": {"start": start, "target": target, "polygons": polygons},
        "pieces": pieces,
        "solutions": solutions,
        "reference": min(range(len(solutions)), key=lambda index: solutions[index]["length"]),
    }
...
def export(destination: Path) -> None:
    ...
    for order in itertools.permutations(range(4)):
        ...
        process = subprocess.run(
            [main.SOLVER_BINARY],
            input=main.live_solver_input(case, 200000, 3),
            text=True,
            capture_output=True,
            check=True,
            timeout=8,
    ...
    data = {
        "geometry": {"start": start, "target": target, "polygons": polygons},
        "solutions": solutions,
        "reference": min(range(24), key=lambda index: solutions[index]["length"]),
        "provenance": "Native fixed-order solver, all 24 permutations; numerical solutions. Synthetic teaching example, separate from the research suite.",
    }
