...
def compact_event(event: dict) -> dict:
	...
	fields = (
		"kind", "node", "parent", "polygon", "piece", "position", "pass", "sequence", "order",
		"path", "lower_bound", "upper_bound", "length", "pruned", "source", "reason",
	)
...
def export_case(case: GermanCase, solver: Path, max_seconds: float, max_events: int) -> dict:
	...
	if max_events > 0 and len(events) > max_events:
		...
		search = [event for event in events if not event["kind"].startswith("heuristic") and event["kind"] != "incumbent" and event["kind"] != "complete"]
	...
	result_summary = {
		key: result[key]
		for key in (
			"exact", "termination", "lower_bound", "upper_bound", "calls", "nodes",
			"screened_nodes", "insertion_branches", "decomposition_branches",
		)
		if key in result
	}
	return {
