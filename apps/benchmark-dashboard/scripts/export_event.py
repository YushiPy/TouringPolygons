...
def export_dataset(run: Path) -> dict:
	...
	return {
		...
		"config": {key: manifest[key] for key in (
			"threads", "max_seconds", "max_calls", "absolute_gap", "relative_gap",
			"solver_visit_tolerance", "independent_geometry_tolerance",
		)},
		"summary": summary,
		"independent_validation": {
			key: independent[key] for key in (
				"socp_cases", "max_socp_objective_difference", "socp_nonfinite_lower_bound_cases",
				"independent_interruptions", "interruption_termination_counts",
			)
		},
		"notes": [
