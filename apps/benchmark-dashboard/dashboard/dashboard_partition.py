...
def native_partition(polygons: list[list[list[float]]]) -> list:
    ...
    sources = [
        APP / "scripts/partition_cli.cpp",
        LIB / "src/optimal_convex_partition.cpp",
        LIB / "include/optimal_convex_partition/optimal_convex_partition.h",
    ]
    with LOCK:
    ...
    def partition(request: PartitionRequest):
        ...
        try:
            return {
                ...
                "pieces": native_partition(request.polygons),
            }
